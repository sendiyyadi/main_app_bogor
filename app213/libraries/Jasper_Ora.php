    ...
    function cetak_ora($rpt, $param_data = array(), $type="pdf", $ignore_html_pg=TRUE) {
      ...
      //  $reportx    = "{$rpt}.jrxml";
        ...
        if ($type=="html") {
            ...
            // gunakan XLSX exporter yang jauh lebih stabil
            $exporter = new java("net.sf.jasperreports.engine.export.ooxml.JRXlsxExporter");

            // set parameters baru (lebih simpel)
            $exporter->setParameter(
                java("net.sf.jasperreports.engine.JRExporterParameter")->JASPER_PRINT,
                $jasperPrint
            );

            $exporter->setParameter(
                java("net.sf.jasperreports.engine.JRExporterParameter")->OUTPUT_FILE_NAME,
                $outputPath . ".xlsx"
            );
            ...
            readfile($outputPath . ".xlsx");
            @unlink($outputPath . ".xlsx");
            $conn->close();
            return;

...
class piutang_nop extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        //$this->load->model('kecModel', 'kec_model');
        //$this->load->model('kelModel', 'kel_model');
    ...
    public function index()
    {
        ...
        //-----------------------------------------------------------------------      
        //$kec_kd = (isset($_GET['kec_kd']) ? $_GET['kec_kd'] : '000');
        //$select_data = $this->kec_model->getRecord($kec_kd);
        $select_data = $this->get_select_kec();

        $options = array();
        $options['000'] = 'SEMUA KECAMATAN';
        ...
        $data['select_kec'] = preg_replace("/[\r\n]+/", "", form_dropdown('kec_kd', $options, $kec_kd, $js));
        //-----------------------------------------------------------------------
        //$kel_kd = (isset($_GET['kel_kd']) ? $_GET['kel_kd'] : '000');
        //$select_data = $this->kel_model->getRecord($kec_kd, $kel_kd);
        $select_data = $this->get_select_kel($kec_kd);
        $options     = array();
        $options['000'] = 'SEMUA KELURAHAN';
        foreach ($select_data as $row) {
            $options[$row->kd_kelurahan] = $row->nm_kelurahan;
        }
        $js = 'id="kel_kd" name="kel_kd" class="input form-control select2" style="width: 250px" ';
        $data['select_kel'] = preg_replace("/[\r\n]+/", "", form_dropdown('kel_kd', $options, $kel_kd, $js));
        //-----------------------------------------------------------------------
        // load
        ...
        $data['data_source'] = active_module_url() . "piutang_nop/grid?nama_wp={$nama_wp}&alamat_op={$alamat_op}&nop_pbb={$nop_pbb}&kec_kd={$kec_kd}&kel_kd={$kel_kd}";
    ...
    function grid()
    {
        ...
        if (strlen($nop) == 18) {
            ...
            $prop_op = substr($nop, 0, 2);
            $kab_op  = substr($nop, 2, 2);
            $kec_op  = substr($nop, 4, 3);
            $kel_op  = substr($nop, 7, 3);
            $blok_op = substr($nop, 10, 3);
            $urut_op = substr($nop, 13, 4);
            $jns_op  = substr($nop, 17, 1);

            $filter .= "and s.kd_kecamatan = '{$kec_op}'
            and s.kd_kelurahan = '{$kel_op}'
            and s.kd_blok = '{$blok_op}'
            and s.no_urut = '{$urut_op}'
            and s.kd_jns_op = '{$jns_op}'";
        }
        // jika nop isi dan jml bukan 18 , maka filter batal semua
        if (!empty($nop) && strlen($nop) != 18) {
            $filter = "";
        } else if ($kec_kd == '999' or $kel_kd == '999') {
            $filter = "";
        } else if ($kec_kd !== '000' && $kel_kd == '000') {
            $filter .= " and s.kd_kecamatan = '{$kec_kd}' ";
        } else if ($kec_kd !== '000' && $kel_kd !== '000') {
            $filter .= " and s.kd_kecamatan = '{$kec_kd}' and s.kd_kelurahan = '{$kel_kd}' ";
        }
        //log_message('info', " SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS loaddata piutang_pbb KODE : " .$filter." kec_kd ".$kec_kd." kel_kd ".$kel_kd);      
        ...
        if (empty($filter)) {
            $sql_query_r = "select 1 as ctr from dual where 1=2";
        }
        //
        $output = array(
            "sEcho" => $sEcho,
        ...
        // var_dump($sql_query_r);
        // die;
        $qry = $this->db->query($sql_query_r)->result();
            ...
            foreach ($qry as $aRow) {
                ...
                for ($i = 0; $i < count($aColumns); $i++) {
                    ...
                    if ($aColumns[$i] == 'nop') {
                        ...
                        ";
                        */
                    } else if ($aColumns[$i] == 'nm_kec_op') {
                        $row[] = $aRow->NM_KEC_OP;
                    } else if ($aColumns[$i] == 'nm_kel_op') {
                        $row[] = $aRow->NM_KEL_OP;
                    } else if ($aColumns[$i] == 'thn_pajak_sppt') {
                        $row[] = $aRow->THN_PAJAK_SPPT;
                    } else if ($aColumns[$i] == 'alamat_op') {
                        $row[] = $aRow->ALAMAT_OP;
                    } else if ($aColumns[$i] == 'ketetapan') {
                        $row[] = number_format($aRow->KETETAPAN, 0, ',', '.');
                    } else if ($aColumns[$i] == 'pokok') {
                        $row[] = number_format($aRow->POKOK, 0, ',', '.');
                    } else if ($aColumns[$i] == 'pengurangan') {
                        $row[] = number_format($aRow->PENGURANGAN, 0, ',', '.');
                    } else if ($aColumns[$i] == 'denda') {
                        $row[] = number_format($aRow->DENDA, 0, ',', '.');
                    } else if ($aColumns[$i] == 'bayar') {
                        $row[] = number_format($aRow->JML_BAYAR, 0, ',', '.');
                    } else {
                        $row[] = number_format($sisa_ar, 0, ',', '.');
                    } // sisa ar / piutang
    ...
    function get_select_kec()
    {
        ...
        $sql = "select * from ref_kecamatan where kd_propinsi='" . KD_PROPINSI . "' and kd_dati2='" . KD_DATI2 . "' order by nm_kecamatan";
        $qry = $this->db_pbbm->query($sql);
        return $qry->result();
    }

    function get_select_kel($kec = '000')
    {
        $sql = "select * from ref_kelurahan where kd_propinsi='" . KD_PROPINSI . "' and kd_dati2='" . KD_DATI2 . "' and kd_kecamatan='$kec' order by nm_kelurahan";
        $qry = $this->db_pbbm->query($sql);
        return $qry->result();
    }
}

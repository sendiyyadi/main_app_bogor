...
class Datatables_ora
{
   ...
   // protected $ci;
    ...
    protected $encript_columns = array(); // arig 2020.12.12
    protected $rupiah_columns_d2 = array();
    protected $rupiah_columns_d3 = array();    
    ...
    public function __construct()
    {
      //  $this->ci =& get_instance();
        $this->oraci =& get_instance();
        $this->oradb = $this->oraci->load->database('oradb', TRUE); 
    }
    ...
    public function select($columns, $backtick_protect = TRUE)
    {
        ...
        //$this->oraci->oradb->select($columns, $backtick_protect);
        $this->oradb->select($columns, $backtick_protect);
        return $this;
    }
    ...
    public function where($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
        ...
        $this->oradb->where($key_condition, $val, $backtick_protect);
        return $this;
    }
	...
	*/
    ...
    public function rupiah_column_d2($columns)
    { // arig 2018.08.01
        foreach ($this->explode(',', $columns) as $val) {
            $column                 = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
            $this->rupiah_columns_d2[] = $column;
        }
        return $this;
    }

    public function rupiah_column_d3($columns)
    { // arig 2018.08.01
        foreach ($this->explode(',', $columns) as $val) {
            $column                 = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
            $this->rupiah_columns_d3[] = $column;
        }
        return $this;
    }
    ...
    public function encript_column($columns)
    {
        foreach ($this->explode(',', $columns) as $val) {
            $column                  = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$2', $val));
            $this->encript_columns[] = $column;
        }
        return $this;
    }
    ...
    public function generate_arig($charset = 'UTF-8')
    {
        //$this->get_paging();
        $this->get_ordering();
        $this->get_filtering();
    ...
    protected function get_paging()
    {
        ...
        $iStart  = $this->oraci->input->get('iDisplayStart');
        $iLength = $this->oraci->input->get('iDisplayLength');

        $this->oradb->limit(($iLength != '' && $iLength != '-1') ? $iLength : 10, ($iStart) ? $iStart : 0);
    }
    ...
    protected function get_ordering()
    {
        ...
        elseif ($this->oraci->input->get('sColumns')){
            //$mColArray = explode(',', $this->ci->input->get('sColumns'));
            $mColArray = explode(',', $this->oraci->input->get('sColumns'));
        }
        else{
        ...
        for ($i = 0; $i < intval($this->oraci->input->get('iSortingCols')); $i++)
            if (isset($mColArray[intval($this->oraci->input->get('iSortCol_' . $i))]) && in_array($mColArray[intval($this->oraci->input->get('iSortCol_' . $i))], $columns) && $this->oraci->input->get('bSortable_' . intval($this->oraci->input->get('iSortCol_' . $i))) == 'true')
                $this->oradb->order_by($mColArray[intval($this->oraci->input->get('iSortCol_' . $i))], $this->oraci->input->get('sSortDir_' . $i));
    }
    ...
    protected function get_filtering()
    {
        ...
        $sWhere  = '';
        //
        $sSearch = $this->oraci->input->get('sSearch');

        $mColArray = array_values(array_diff($mColArray, $this->unset_columns));
        $columns   = array_values(array_diff($this->columns, $this->unset_columns));
		
		$db_driver = 'oci8';//DB_DRIVER ;  // arig 2018-09-19
        //if ($sSearch != '')
        if ($sSearch != '' && $db_driver == 'oci8') {  // arig 2018-09-19
            for ($i = 0; $i < count($mColArray); $i++)
                if ($this->oraci->input->get('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)){
                    $sWhere .= "upper(".$this->select[$mColArray[$i]] . ") LIKE upper('%" . $sSearch . "%') OR ";
                }
        }
        //
        //if ($sSearch != '') {
            ...
            for ($i = 0; $i < count($mColArray); $i++) {
                if ($this->oraci->input->get('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)) {
					if (@in_array($mColArray[$i], $date_columns)) {
						...
						$sWhere .= $this->select[$mColArray[$i]] . "::text = '" . $sSearch . "' OR ";
                    } else if (@in_array($mColArray[$i], $rupiah_columns_d2)) {
                        $sWhere .= $this->select[$mColArray[$i]] . "::text = '" . $sSearch . "' OR ";
                    } else if (@in_array($mColArray[$i], $rupiah_columns_d3)) {
                        $sWhere .= $this->select[$mColArray[$i]] . "::text = '" . $sSearch . "' OR ";                        
					} else {
        ...
        if ($sWhere != '')
            $this->oradb->where('(' . $sWhere . ')');

        for ($i = 0; $i < intval($this->oraci->input->get('iColumns')); $i++) {
            if ($this->oraci->input->get('sSearch_' . $i) && $this->oraci->input->get('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns)) {
                $miSearch = explode(',', $this->oraci->input->get('sSearch_' . $i));
                foreach ($miSearch as $val) {
                    if (preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
                        $this->oradb->where($this->select[$mColArray[$i]] . ' ' . $matches[1], $matches[3]);
                    else
                        $this->oradb->where($this->select[$mColArray[$i]] . ' LIKE', '%' . $val . '%');
                }
            }
        }

        foreach ($this->filter as $val)
            $this->oradb->where($val[0], $val[1], $val[2]);
    }
	...
	//
        ...
        for ($i = 0; $i < sizeof($aaData); $i++) {
            ...
            for ($j = 0; $j < sizeof($this->rupiah_columns_d2); $j++) {
                $aaData[$i][$this->rupiah_columns_d2[$j]] = number_format($aaData[$i][$this->rupiah_columns_d2[$j]], 2, ',', '.');
            }

            for ($j = 0; $j < sizeof($this->rupiah_columns_d3); $j++) {
                $aaData[$i][$this->rupiah_columns_d3[$j]] = number_format($aaData[$i][$this->rupiah_columns_d3[$j]], 3, ',', '.');
            }
            ...
            for ($j = 0; $j < sizeof($this->checkbox_columns); $j++) {
                ...
                $aaData[$i][$this->checkbox_columns[$j]] = $aaData[$i][$this->checkbox_columns[$j]] == 1 ? '<i class="fa fa-check"></i>' : '';
            }

            for ($j = 0; $j < sizeof($this->encript_columns); $j++) {
                $aaData[$i][$this->encript_columns[$j]] = get_encript($aaData[$i][$this->encript_columns[$j]]);
            }
        ...
        $sOutput = array(
			'Query' => $qry,
            'sEcho' => intval($this->oraci->input->get('sEcho')),
    ...
    protected function get_total_results($filtering = FALSE)
    {
        ...
        foreach ($this->joins as $val)
            $this->oradb->join($val[0], $val[1], $val[2]);

        foreach ($this->where as $val)
            $this->oradb->where($val[0], $val[1], $val[2]);

        return $this->oradb->count_all_results($this->table);
    }
    ...
    {
        if (!$this->oraci->input->get('mDataProp_0'))
            return FALSE;

        for ($i = 0; $i < intval($this->oraci->input->get('iColumns')); $i++)
            if (!is_numeric($this->oraci->input->get('mDataProp_' . $i)))
                return TRUE;
    ...
    protected function get_mDataprop()
    {
        ...
        for ($i = 0; $i < intval($this->oraci->input->get('iColumns')); $i++)
            $mDataProp[] = $this->oraci->input->get('mDataProp_' . $i);

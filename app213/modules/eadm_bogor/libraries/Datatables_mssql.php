...
class Datatables_mssql
{
    ...
    public function __construct() 
    {
        ...
        $this->mssqlci =& get_instance();
        $this->mssqldb = $this->mssqlci->load->database('mssqldb', TRUE);         
    }
    ...
    public function select($columns, $backtick_protect = TRUE)
    {
        ...
        $this->mssqldb->select($columns, $backtick_protect);
        return $this;
    }
    ...
    public function join($table, $fk, $type = NULL)
    {
        ...
        $this->mssqldb->join($table, $fk, $type);
        return $this;
    }
    ...
    public function where($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
        ...
        $this->mssqldb->where($key_condition, $val, $backtick_protect);
        return $this;
    }
    ...
    */
    ...
    {
        $iStart  = $this->mssqlci->input->get('iDisplayStart');
        $iLength = $this->mssqlci->input->get('iDisplayLength');
        $this->mssqldb->limit(($iLength != '' && $iLength != '-1') ? $iLength : 10, ($iStart) ? $iStart : 0);
    }
    ...
    protected function get_ordering()
    {
        ...
        elseif ($this->mssqlci->input->get('sColumns'))
            $mColArray = explode(',', $this->mssqlci->input->get('sColumns'));
        else
        ...
        for ($i = 0; $i < intval($this->mssqlci->input->get('iSortingCols')); $i++)
            if (isset($mColArray[intval($this->mssqlci->input->get('iSortCol_' . $i))]) && in_array($mColArray[intval($this->mssqlci->input->get('iSortCol_' . $i))], $columns) && $this->mssqlci->input->get('bSortable_' . intval($this->mssqlci->input->get('iSortCol_' . $i))) == 'true')
                $this->mssqldb->order_by($mColArray[intval($this->mssqlci->input->get('iSortCol_' . $i))], $this->mssqlci->input->get('sSortDir_' . $i));
    }
    ...
    protected function get_filtering()
    {
        ...
        $sWhere  = '';
        //eon_beji
        //$sSearch = mysql_real_escape_string($this->mssqlci->input->get('sSearch'));
        $sSearch = $this->mssqlci->input->get('sSearch');
        ...
        if ($sSearch != '')
            for ($i = 0; $i < count($mColArray); $i++)
                if ($this->mssqlci->input->get('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns))
        ...
        */
        ...
        if ($sWhere != '')
            $this->mssqldb->where('(' . $sWhere . ')');

        for ($i = 0; $i < intval($this->mssqlci->input->get('iColumns')); $i++) {
            if ($this->mssqlci->input->get('sSearch_' . $i) && $this->mssqlci->input->get('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns)) {
                $miSearch = explode(',', $this->mssqlci->input->get('sSearch_' . $i));
                foreach ($miSearch as $val) {
                    if (preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
                        $this->mssqldb->where($this->select[$mColArray[$i]] . ' ' . $matches[1], $matches[3]);
                    else
                        $this->mssqldb->where($this->select[$mColArray[$i]] . ' LIKE', '%' . $val . '%');
                }
            }
        }

        foreach ($this->filter as $val)
            $this->mssqldb->where($val[0], $val[1], $val[2]);
    }
    ...
    //eon_beji
        ...
        //log_message('info', "KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK : ". $qry);
        $sOutput = array(
            'Query' => $qry,
            'sEcho' => intval($this->mssqlci->input->get('sEcho')),
    ...
    protected function get_total_results($filtering = FALSE)
    {
        ...
        foreach ($this->joins as $val)
            $this->mssqldb->join($val[0], $val[1], $val[2]);

        foreach ($this->where as $val)
            $this->mssqldb->where($val[0], $val[1], $val[2]);

        return $this->mssqldb->count_all_results($this->table);
    }
    ...
    {
        if (!$this->mssqlci->input->get('mDataProp_0'))
            return FALSE;

        for ($i = 0; $i < intval($this->mssqlci->input->get('iColumns')); $i++)
            if (!is_numeric($this->mssqlci->input->get('mDataProp_' . $i)))
                return TRUE;
    ...
    protected function get_mDataprop()
    {
        ...
        for ($i = 0; $i < intval($this->mssqlci->input->get('iColumns')); $i++)
            $mDataProp[] = $this->mssqlci->input->get('mDataProp_' . $i);

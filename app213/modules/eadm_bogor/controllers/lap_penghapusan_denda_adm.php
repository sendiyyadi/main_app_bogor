...
class lap_penghapusan_denda_adm extends CI_Controller
{
    private $controller = 'lap_penghapusan_denda_adm';
    ...
    function __construct()
    {
        ...
        $module = 'lap_penghapusan_denda_adm';
        $this->load->library('module_auth', array(
            'module' => $module
        ));

        $this->load->model(array(
            'apps_model', 'lap_penghapusan_denda_adm_model'
        ));
    ...
    public function index()  {
        ...
        $data['page_menu'] = 'lap_penghapusan_denda_adm';
        $data['current'] = 'lap_penghapusan_denda_adm';
        $data['apps']    = $this->apps_model->get_active_only();

        $select_data  = $this->load->model('lap_penghapusan_denda_adm_model')->get_select_kecamatan();
                $options      = array();
                $kec_id = '';
                if($select_data) {
        ...
        $this->load->view('vlap_penghapusan_denda_adm', $data);
    }

    function get_kelurahan() {
        //log_message('info', "sssssssssssssssssssssssssssssssss");
        $kec_id    = $this->uri->segment(4);
        $kelurahan = $this->load->model('lap_penghapusan_denda_adm_model')->get_select_kel($kec_id);
    ...
    function cetak() {
        ...
        $rpt = "lap_penghapusan_denda_adm_3";

        $query = $this->load->model('lap_penghapusan_denda_adm_model')->query_rpt($filter);
    ...
    function cetak_rpt() {
        ...
        $filter .= " and byr.thn_pajak_sppt BETWEEN '1994' AND '2025' ";

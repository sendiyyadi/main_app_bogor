...
class nop_prog extends CI_Controller
{
    ...
    public function grid()
    {
       ...
       $tgl_end = date('Ymd',strtotime($this->input->get('tgl_end')));
        ...
        $this->datatables->select("RS.KD_PROPINSI||RS.KD_DATI2||RS.KD_KECAMATAN||RS.KD_KELURAHAN||RS.KD_BLOK||RS.NO_URUT||RS.KD_JNS_OP||TRIM(RS.NIK) AS IDX ,ROW_NUMBER() OVER (ORDER BY RS.KD_PROPINSI||'.'||RS.KD_DATI2||'-'||RS.KD_KECAMATAN||'.'||RS.KD_KELURAHAN||'-'||RS.KD_BLOK||'.'||RS.NO_URUT||'.'||RS.KD_JNS_OP) as nomer, RS.NIK, RS.KD_PROPINSI||'.'||RS.KD_DATI2||'-'||RS.KD_KECAMATAN||'.'||RS.KD_KELURAHAN||'-'||RS.KD_BLOK||'.'||RS.NO_URUT||'.'||RS.KD_JNS_OP AS NOPS, RS.NAMA, '' as NO_PERMOHONAN, CASE WHEN RS.STATUS=0 then 'Draft (0)' WHEN RS.STATUS=1 then 'Terima (1)' WHEN RS.STATUS=2 then 'Tolak (2)' END AS sts, RS.STATUS, TO_CHAR(RS.CREATE_TIME,'DD-MM-YYYY') AS TGL_PERMOHONAN", false);
        $this->datatables->from("REG_ESPPT RS");
        // $this->datatables->join('REG_ESPPT_D RD','RD.KD_PROPINSI=RS.KD_PROPINSI AND RD.KD_DATI2=RS.KD_DATI2 AND RD.KD_KECAMATAN=RS.KD_KECAMATAN AND RD.KD_KELURAHAN=RS.KD_KELURAHAN AND RD.KD_BLOK=RS.KD_BLOK AND RD.NO_URUT=RS.NO_URUT AND RD.KD_JNS_OP=RS.KD_JNS_OP AND RD.NIK=RS.NIK','INNER');
        ...
        //// cek  usr_sppt is null and create_time is not null
        //$this->datatables->where('RS.USR_SPPT IS NULL');
        $this->datatables->where('RS.CREATE_TIME IS NOT NULL');
    ...
    public function action()
    {
        ...
        if ($p_id && $get = $this->nop_prog_model->get_view($p_id)) {
            ...
            $data['p_id'] = $p_id;
            $data['dt']['ket_tolak'] = '';
            $tidak_ada = '<p class="mb-0">File Tidak Ada</p>';
   ...
   public function tolak()
    {
        ...
        if($p_id == $po_id){
            ...
            $config = array(
                'protocol' => 'smtp',
                'smtp_host' => 'ssl://smtp.googlemail.com', //sesuaikan dengan host pengirim
                'smtp_port' => 465,
                'smtp_timeout' => 20,
                'smtp_user' => 'subbidpsi2022@gmail.com', //sesuaikan dengan email yg dipakai
                'smtp_pass' => 'pldvtoochjjxyhor', //password host
                'smtp_username' => 'REG ESPPT KABUPATEN BOGOR',
                'mailtype' => 'html',
                'charset' =>'iso-8859-1',
                'wordwrap' => true,
            );

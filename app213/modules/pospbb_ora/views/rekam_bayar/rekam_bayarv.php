            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-0">Rekam Bayar (Bendahara)</h4>
                        <div class="page-title-right" id="test">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item">
                                    <a href="javascript: void(0);">STTS</a>
                                </li>
                                <li class="breadcrumb-item active">Rekam Bayar (Bendahara)</li>
                            </ol>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="row">
                                <div class="col-sm-12 col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <input class="form-control" type="text" name="jml_byr" id="jml_byr">
                                    </div>
                                </div>
                                <div class="col-sm-12 col-md-4">
                                    <div class="mb-3">
                                        <label class="form-label">Tanggal Pembayaran</label>
                                        <input class="form-control" type="text" name="tgl_byr" id="tgl_byr" value="<?= date('d-m-Y') ?>">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-sm-12 col-md-4">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control select2" id="kdtp" name="kdtp">
                                            <option value="77">BJB TRANSFER</option>
                                            <option value="88">BRI ATM</option>
                                            <option value="89">BJB ATM/ONLINE</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function f_cari(nop, thn) {
            // var tgl_byr = $("#tgl_byr").val();

            var tgl_byr_2 = tgl_byr_dtp.date;
            var d = new Date(tgl_byr_2),
                month = '' + (d.getMonth() + 1),
                day = '' + d.getDate(),
                year = d.getFullYear();
            ...
            if (day.length < 2)
                day = '0' + day;
            var jadi = [day, month, year].join('-');
            // alert(jadi);

            $.ajax({
                url: "<?php echo active_module_url('rekam_bayar/cari') ?>" + nop + '/' + thn + '/' + jadi,
                success: function(json) {
                    ...
                    if (data['found'] != 0) {
                        ...
                        $("#pengurangan").val(formatter.format(data['nilai_pengurang']));
        ...
        $('#btn_bayar').click(function() {
            var okegaknih = confirm('Pastikan data sudah benar. Data Pembayaran akan di proses?');
            if (okegaknih == true) {

                $('#myform').submit();
                var jml_byr = parseFloat($('#jml_byr').autoNumeric('get'));
                jml_byr = isNaN(jml_byr) ? 0 : jml_byr;
                var utang = $('#utang').val();
                utang = utang.replace('.', '');
                utang = parseInt(utang);
                utang = isNaN(utang) ? 0 : utang;
                if (jml_byr >= utang) {
        ...
        $('#btn_cetak').click(function() {
            ...
            var thn = $("#tahun").val();
            var ke = $("#ke").val();
            window.open("<?php echo active_module_url('rekam_bayar/cetak') ?>" + nop + '/' + thn + '/' + ke, "Cetak");
            // $(this).attr('disabled', 'disabled');
        });
        ...
        $('#btn_cetak2').click(function() {
            ...
            var thn = $("#tahun").val();
            var ke = $("#ke").val();
            window.open("<?php echo active_module_url('rekam_bayar/cetak_draft') ?>" + nop + '/' + thn + '/' + ke, "Cetak Bank");
            // $(this).attr('disabled', 'disabled');
        });
        ...
        $('#btn_cetak3').click(function() {
            ...
            var thn = $("#tahun").val();
            var ke = $("#ke").val();
            window.open("<?php echo active_module_url('rekam_bayar/cetak_bank') ?>" + nop + '/' + thn + '/' + ke, "Cetak Bank2");
            // $(this).attr('disabled', 'disabled');
        });

        var nowTemp = new Date();
        var now = new Date(nowTemp.getFullYear(), nowTemp.getMonth(), nowTemp.getDate(), 0, 0, 0, 0);
        var tgl_byr_dtp = $('#tgl_byr').datepicker({
            format: 'dd-mm-yyyy',

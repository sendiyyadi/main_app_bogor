...
class pos_user_model extends CI_Model
{
    ...
    function get_k($id)
    {
        $sql = "select * from SEC_USERS where USERID='{$id}'";
    ...
    function get_q($id)
    {
        $sql = "select * from SEC_USERS where ID={$id}";
    ...
    function get_user_pbb($id)
    {
        ...
        (tp.KD_KANWIL||tp.KD_KANTOR||tp.KD_TP) as GRP_TP, tp.NM_TP, US.PASSWD, US.DISABLED
        from SEC_USERS us
        left join V_USER_PBB up on up.USERID=us.USERID
        left join S_TEMPAT_PEMBAYARAN tp on {$join}
        where us.id={$id} ";

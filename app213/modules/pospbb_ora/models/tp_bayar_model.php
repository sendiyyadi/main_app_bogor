...
class tp_bayar_model extends CI_Model
{
	...
	function get_all()
	{
		...
		$sql = "SELECT KD_KANWIL||KD_KANTOR||KD_TP as KODE, KD_KANWIL, KD_KANTOR, KD_TP, NM_TP, ALAMAT_TP, NO_REK_TP
        FROM S_TEMPAT_PEMBAYARAN TP order by NM_TP ";
	...
	{
		$schema_pbb = $this->schema_pbb;
		$sql = "select KD_KANWIL||KD_KANTOR||KD_TP AS KODE, s.*
        from S_TEMPAT_PEMBAYARAN s
        where KD_KANWIL||KD_KANTOR||KD_TP = '$id'";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0) {
			return $query->row();
		} else {
	...
	{
		$this->db->trans_start();
		$this->db->where('KD_KANWIL||KD_KANTOR||KD_TP', $id);
	...
	{
		$this->db->trans_start();
		$this->db->where('KD_KANWIL||KD_KANTOR||KD_TP', $data);

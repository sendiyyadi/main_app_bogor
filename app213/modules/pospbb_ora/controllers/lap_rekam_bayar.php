...
class lap_rekam_bayar extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $module = 'LAP_RKM_BYR';  // POS PENGURANGAN COVID 19
	...
	*/
        //------------------------------------------------------------------
        $select_data = $this->load->model('lap_rekam_bayar_model')->get_select_tp();
        $options     = array();
        foreach ($select_data as $row) {
            $options[$row->KD_TP] = $row->NM_TP;
        }
        $js  = 'id="kd_tp" class="input form-control select2" ';
        $select = form_dropdown('kd_tp', $options, ' ', $js);
        $select = preg_replace("/[\r\n]+/", "", $select);
        $data['select_tp'] = $select;
        //------------------------------------------------------------------
        $select_data = $this->load->model('lap_rekam_bayar_model')->get_select_kec();
        $options     = array();
        ...
        //------------------------------------------------------------------
        $select_data = $this->load->model('lap_rekam_bayar_model')->get_select_kel(' ');
        $options     = array();
        if ($select_data) {
        ...
        $this->load->view('lap_rekam_bayar/vlap_rekam_bayar', $data);
    }
    ...
    function get_kelurahan()
    {
        ...
        $kelurahan = $this->load->model('lap_rekam_bayar_model')->get_select_kel($kec_id);

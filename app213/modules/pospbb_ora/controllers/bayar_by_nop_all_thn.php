...
class bayar_by_nop_all_thn extends CI_Controller
{
    private $module = 'bayar_by_nop_all_thn';
    ...
    public function index()
    {
        ...
        $data['faction'] = active_module_url('bayar_by_nop_all_thn/simpan');
        //$data['current'] = 'stts';

        $this->load->view('bayar_by_nop_all_thn/vbayar_by_nop_all_thn', $data);
    }
    ...
    public function cari()
    {
        ...
        $nopd = $this->uri->segment(4);
        // $range2 = $this->uri->segment(5);
        // $thn    = $this->uri->segment(6);

        if ($nopd && $result = $this->sppt_model->get_by_nop($nopd)) {
            $output = array(
                'found' => 1,
                //'sEcho' => intval($sEcho),
            ...
            foreach ($result['query'] as $data) {
                ...
                if ($jtempo < date('Y-m-d')) {
                    ...
                    if (KD_PROPINSI == '32' && KD_DATI2 == '03') {
                        ...
                        // if ((int)$thn_pjk <= 2021 && $today <= 20210630 && $today >= 20210401) {
                        //     $denda = 0;
                        // }
                ...
                // if (KD_PROPINSI == '32' && KD_DATI2 == '171' && $thn_pjk == 2020 && $faktor_pengurang_sppt == 0 && $today <= 20210630 && $today >= 20210401) {
    ...
    private function fvalidation()
    {
        ...
        $this->form_validation->set_rules('range1', 'NOP', 'required');
                ...
                foreach ($data as $row) {
                    ...
                    if ($query = $this->sppt_model->get_by_nop_thn($nop, $thn_pajak_sppt)) {
                        ...
                        if ($jtempo < date('Y-m-d')) {
                            ...
                            if (KD_PROPINSI == '32' && KD_DATI2 == '03') {
                                // jika Thn Pajak di bawah 2015 dan pemby di dlm range 01-02-2020 sd 30-06-2020
                                // if ((int)$thn_pajak_sppt <= 2099 && $today <= 20210630 && $today >= 20210401) {
                                //     $denda = 0;
                                // }

                                //relaksasi 04062025 rican
                                if ((int)$thn_pajak_sppt == 2025 && $today <= 20250831 && $today >= 20200201 && $query->FAKTOR_PENGURANG_SPPT == 0 && $sisa > 0) {
                                    $denda = 0;
                                }

                                if ((int)$thn_pajak_sppt >= 2020 && (int)$thn_pajak_sppt <= 2024 && $query->FAKTOR_PENGURANG_SPPT == 0 && $today <= 20250630 && $today >= 20250603 && $sisa > 0) {
                                    $denda = 0;
                                }

                                if ((int)$thn_pajak_sppt >= 2020 && (int)$thn_pajak_sppt <= 2024 && $query->FAKTOR_PENGURANG_SPPT == 0 && $today <= 20250731 && $today >= 20250701 && $sisa > 0) {
                                    $denda = 0;
                                }

                                if ((int)$thn_pajak_sppt >= 2020 && (int)$thn_pajak_sppt <= 2024 && $query->FAKTOR_PENGURANG_SPPT == 0 && $today <= 20250831 && $today >= 20250801 && $sisa > 0) {
                                    $denda = 0;
                                }

                                if ((int)$thn_pajak_sppt >= 2012 && (int)$thn_pajak_sppt <= 2019 && $query->FAKTOR_PENGURANG_SPPT == 0 && $today <= 20250630 && $today >= 20250603 && $sisa > 0) {
                                    $denda = 0;
                                }

                                if ((int)$thn_pajak_sppt >= 2012 && (int)$thn_pajak_sppt <= 2019 && $query->FAKTOR_PENGURANG_SPPT == 0 && $today <= 20250731 && $today >= 20250701 && $sisa > 0) {
                                    $denda = 0;
                                }

                                if ((int)$thn_pajak_sppt >= 2012 && (int)$thn_pajak_sppt <= 2019 && $query->FAKTOR_PENGURANG_SPPT == 0 && $today <= 20250831 && $today >= 20250801 && $sisa > 0) {
                                    $denda = 0;
                                }
                            }
                        }
                        ...
                        if (KD_PROPINSI == '32' && KD_DATI2 == '00' && $thn_pajak_sppt == 2020 && $faktor_pengurang_sppt == 0 && $today <= 20210630 && $today >= 20210401) {
                            $denda         = $denda + $denda_baru;
                            $utang         = $sisa + $denda;
                        ...
                        elseif ($thn_pajak_sppt >= 2012 && $thn_pajak_sppt <= 2019 && $faktor_pengurang_sppt == 0 && $today <= 20250831 && $today >= 20250801 && $sisa > 0) {
                            ...
                            $utang     = $sisa + $denda;
                            $terbilang = terbilang($utang);
                            // $nil_pengurang = $faktor_pengurang_sppt;
                            $nil_pengurang = 0;
                            $utang = $utang - $nil_pengurang;
                            //$sisa = $sisa - $nil_pengurang - $denda;

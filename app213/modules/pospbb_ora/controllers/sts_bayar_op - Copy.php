...
class sts_bayar_op extends CI_Controller
{
    ...
    public function index() {
        ...
        $data['pagetitle'] = 'PosPbbDepok';
        $data['title']     = 'PBB Dashboard';
        
        $data['SEC_content'] = '';
        
        /* Mencari Kode NOP */
        if (isset($_POST['nop_kd']))     { $nop_kd = $_POST['nop_kd'];  } 
        else if (isset($_GET['nop_kd'])) { $nop_kd = $_GET['nop_kd']; } 
        else { $nop_kd = 0; }
        ...
        if ($nop_kd != 0 && strlen($nop) == 18 && $nop_kd!='')  {
            ...
            $data_source    = $this->pospbb_ora_model->informasi_objek_pajak($nama_wp);
        }
        // buat tes
        //$data_source    = $this->pospbb_ora_model->informasi_objek_pajak($nama_wp);
        ...
        $this->load->view('vsts_bayar_op', $data);

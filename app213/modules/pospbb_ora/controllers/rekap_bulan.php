...
class rekap_bulan extends CI_Controller
{
    private $module = 'rekap_bulan';
    ...
    public function index()
    {
        ...
        */
        ...
        $js = 'id="tahun" name="tahun" class="input form-control select2" style="width: 90px" ';
        $data['select_thn_bayar'] = form_dropdown('tahun', $options, $tahun, $js);
        //-----------------------------------------------------------------------
        $options = $range_tahun_sppt;
        ...
        $data['data_source'] = active_module_url() . "rekap_bulan/grid?tahun=$tahun&tahun_sppt1=$tahun_sppt1&tahun_sppt2=$tahun_sppt2&buku=$buku&kec_kd=$kec_kd&kel_kd=$kel_kd&tp_kd=$tp_kd";

        $data['tpnm']    = isset($this->session->userdata['tpnm']) ? $this->session->userdata['tpnm'] : '';
        $data['current'] = $this->module;

        $data['apps']    = $this->apps_model->get_active_only();
        $this->load->view('rekap_bulan/vrekap_bulan', $data);
    }
    ...
    function grid()
    {
        ...
        //log_message('info', " 1111111111111111111111111111111111 loaddata dt_rekap_bln: "  );
        /*
         * Limit
         */
        ...
        // AND ps.kd_propinsi='" . KD_PROPINSI . "' AND ps.kd_dati2='" . KD_DATI2 . "' // buang
        $where = "WHERE extract(year FROM ps.tgl_pembayaran_sppt)= $tahun 
        ...
        if ($buku == "15") {
            // semua buku 1 sd 5
            $field_nilai = "sum(nvl(ps.jml_sppt_yg_dibayar,0) - nvl(ps.denda_sppt,0)) as pokok, 
            sum(ps.denda_sppt) as denda, sum(ps.jml_sppt_yg_dibayar) as bayar";
        } else {
            $field_nilai = " sum(case when spt.pbb_yg_harus_dibayar_sppt between $bukumin AND $bukumax 
            then ps.jml_sppt_yg_dibayar-denda_sppt else 0 end) as pokok,
            sum(case when spt.pbb_yg_harus_dibayar_sppt between $bukumin AND $bukumax then ps.denda_sppt else 0 end) as denda,
            sum(case when spt.pbb_yg_harus_dibayar_sppt between $bukumin AND $bukumax then ps.jml_sppt_yg_dibayar else 0 end) as bayar
            ";
        }
        ...
        //  
        $sql_query_r = "SELECT Extract(month FROM ps.tgl_pembayaran_sppt) as kode,
        {$pos_uraian}||':'||nvl(tp.nm_tp,' ') as uraian, ps.thn_pajak_sppt,
        {$field_nilai}
        FROM S_SPPT spt
        ...
        $where $search
        GROUP BY Extract(month FROM ps.tgl_pembayaran_sppt), 
        ({$pos_uraian}||':'||nvl(tp.nm_tp,' ')), ps.thn_pajak_sppt";
        ...
        // log_message('info', " ZZZZZZZZZZZZZZZZZZZ loaddata dt_rekap_bln: " . $sql_query_r);

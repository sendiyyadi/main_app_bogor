...
class bayar_by_blok_thn extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['current']   = 'bayar_by_blok_thn'; // stts
        ...
        $data['faction'] = active_module_url('bayar_by_blok_thn/simpan');
        //$data['current'] = 'stts';

        $this->load->view('bayar_by_blok_thn/vbayar_by_blok_thn', $data);
    }
    ...
    public function cari()
    {
        ...
        if ($blok && $thn && $result = $this->sppt_model->get_by_blok_thn($blok, $thn)) {
            ...
            foreach ($result['query'] as $data) {
                ...
                //     //$denda         = intval($denda * 5/100); // pengurang 5%
                // } else {
                //     $denda     = $denda + $denda_baru;
                //     $utang     = $sisa + $denda;
                //     $nil_pengurang = 0;
        ...
        if ($nop && $thn && $qry = $this->sppt_model->get_by_blok_thn($nop, $thn)) {
            ...
            foreach ($qry['query'] as $row) {
                ...
                if ($sisa > 0) {
                    ...
                    $resultdb = $this->payment_model->add_bayar_per_nop($nopb, $thn_pajak_sppt, $sisa_sppt, $data, $hist_bayar_dt);

...
class rincian_user extends CI_Controller
{
    private $module = 'rincian_user';
    ...
    public function index()
    {
        ...
        $trantypes         = '1'; // $this->uri->segment(3, 1);
        $data['buku']      = $buku;
        ...
        //$data['usertbl']    = $this->load->model('user_pbb_model')->get_users_pospbb(); //get_all();
        ...
        $data['data_source'] = active_module_url() . "rincian_user/grid?buku=$buku&tahun_sppt1=$tahun_sppt1&tahun_sppt2=$tahun_sppt2&tglawal=$tglawal&tglakhir=$tglakhir&kec_kd=$kec_kd&kel_kd=$kel_kd&user_kd=$user_kd";

        $data['tpnm']    = isset($this->session->userdata['tpnm']) ? $this->session->userdata['tpnm'] : '';

        $data['current'] = $this->module;
        $data['apps']    = $this->apps_model->get_active_only();
        $this->load->view('rincian_user/vrincian_user', $data);
    }
    ...
    function grid()
    {
        ...
        $sql_query_r = "SELECT  
            ...
            spt.kd_blok ||'.'||spt.no_urut||'.'|| spt.kd_jns_op as kode, 
            spt.nm_wp_sppt as uraian, {$pos_uraian}||':'||nvl(tp.nm_tp,' ') as nm_tp, ps.thn_pajak_sppt,
            (coalesce(ps.jml_sppt_yg_dibayar,0) - coalesce(ps.denda_sppt,0)) as pokok, 
            ps.denda_sppt as denda, ps.jml_sppt_yg_dibayar as bayar, 
            to_char(ps.tgl_pembayaran_sppt,'dd-mm-yyyy') as tanggal, nvl(up.NM_LOGIN,' ') as nama
            FROM S_SPPT spt
           ...
           $where $search 
            ...
            spt.kd_blok ||'.'||spt.no_urut||'.'|| spt.kd_jns_op,
            spt.nm_wp_sppt, {$pos_uraian}||':'||nvl(tp.nm_tp,' ') ";

            //var_dump($sql_query_r);die;

        $sql_query_r .= "$sOrder $sLimit";
        // log_message('info', " 44444444444444 loaddata dt_rincian_user " . $sql_query_r);

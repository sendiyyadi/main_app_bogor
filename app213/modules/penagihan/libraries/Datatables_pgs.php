...
class Datatables_pgs
{
    ...
    public function __construct() 
    {
        ...
        $this->pgsci =& get_instance();
        $this->pgsdb = $this->pgsci->load->database('pgsdb', TRUE);         
    }
    ...
    public function select($columns, $backtick_protect = TRUE)
    {
        ...
        $this->pgsdb->select($columns, $backtick_protect);
        return $this;
    }
    ...
    public function where($key_condition, $val = NULL, $backtick_protect = TRUE)
    {
        ...
        $this->pgsdb->where($key_condition, $val, $backtick_protect);
        return $this;
    }
	...
	*/
    ...
    {
        $iStart  = $this->pgsci->input->get('iDisplayStart');
        $iLength = $this->pgsci->input->get('iDisplayLength');
        $this->pgsdb->limit(($iLength != '' && $iLength != '-1') ? $iLength : 10, ($iStart) ? $iStart : 0);
    }
    ...
    protected function get_ordering()
    {
        ...
        elseif ($this->pgsci->input->get('sColumns'))
            $mColArray = explode(',', $this->pgsci->input->get('sColumns'));
        else
        ...
        for ($i = 0; $i < intval($this->pgsci->input->get('iSortingCols')); $i++)
            if (isset($mColArray[intval($this->pgsci->input->get('iSortCol_' . $i))]) && in_array($mColArray[intval($this->pgsci->input->get('iSortCol_' . $i))], $columns) && $this->pgsci->input->get('bSortable_' . intval($this->pgsci->input->get('iSortCol_' . $i))) == 'true')
                $this->pgsdb->order_by($mColArray[intval($this->pgsci->input->get('iSortCol_' . $i))], $this->pgsci->input->get('sSortDir_' . $i));
    }
    ...
    protected function get_filtering()
    {
        ...
        $sWhere  = '';
        //eon_beji
        //$sSearch = mysql_real_escape_string($this->pgsci->input->get('sSearch'));
        $sSearch = $this->pgsci->input->get('sSearch');
        ...
        if ($sSearch != '')
            for ($i = 0; $i < count($mColArray); $i++)
                if ($this->pgsci->input->get('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns))
        ...
        */
        ...
        if ($sWhere != '')
            $this->pgsdb->where('(' . $sWhere . ')');

        for ($i = 0; $i < intval($this->pgsci->input->get('iColumns')); $i++) {
            if ($this->pgsci->input->get('sSearch_' . $i) && $this->pgsci->input->get('sSearch_' . $i) != '' && in_array($mColArray[$i], $columns)) {
                $miSearch = explode(',', $this->pgsci->input->get('sSearch_' . $i));
                foreach ($miSearch as $val) {
                    if (preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
                        $this->pgsdb->where($this->select[$mColArray[$i]] . ' ' . $matches[1], $matches[3]);
                    else
                        $this->pgsdb->where($this->select[$mColArray[$i]] . ' LIKE', '%' . $val . '%');
                }
            }
        }

        foreach ($this->filter as $val)
            $this->pgsdb->where($val[0], $val[1], $val[2]);
    }
	...
	//eon_beji
        ...
        $sOutput = array(
			'Query' => $qry,
            'sEcho' => intval($this->pgsci->input->get('sEcho')),
    ...
    protected function get_total_results($filtering = FALSE)
    {
        ...
        foreach ($this->joins as $val)
            $this->pgsdb->join($val[0], $val[1], $val[2]);

        foreach ($this->where as $val)
            $this->pgsdb->where($val[0], $val[1], $val[2]);

        return $this->pgsdb->count_all_results($this->table);
    }
    ...
    {
        if (!$this->pgsci->input->get('mDataProp_0'))
            return FALSE;

        for ($i = 0; $i < intval($this->pgsci->input->get('iColumns')); $i++)
            if (!is_numeric($this->pgsci->input->get('mDataProp_' . $i)))
                return TRUE;
    ...
    protected function get_mDataprop()
    {
        ...
        for ($i = 0; $i < intval($this->pgsci->input->get('iColumns')); $i++)
            $mDataProp[] = $this->pgsci->input->get('mDataProp_' . $i);

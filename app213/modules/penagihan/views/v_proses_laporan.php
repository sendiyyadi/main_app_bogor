            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-0">DAFTAR PROSES LAPORAN DISTRIBUSI SPPT</h4>
                        <div class="page-title-right" id="test">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item">
                                    <a href="javascript: void(0);">Distribusi SPPT</a>
                                </li>
                                <li class="breadcrumb-item active">Daftar Proses Laporan Distribusi SPPT</li>
                            </ol>
                        </div>
                    </div>
                </div>
...
function reload_grid() {
  ...
  oTable.fnReloadAjax("<?php echo active_module_url();?>proses_laporan/grid/?"+data_params);
...
function get_kelurahan(kec_id) {
	$.ajax({
		url: "<?php echo active_module_url()?>proses_laporan/get_kelurahan/"+kec_id,
		success: function (j) {
...
function action_terima_sim(form) {
      ...
      form.action = '<?php echo active_module_url()?>proses_laporan/terima_sim/';
...
function action_tolak_sim(form) {
      ...
      form.action = '<?php echo active_module_url()?>proses_laporan/tolak_sim/';
    ...
    $(document).ready(function () {
        ...
        window.history.replaceState({}, "", "<?php echo active_module_url();?>proses_laporan/");
        ...
        oTable = $('#mytable').dataTable({
            ...
            { "aTargets": [1], "bSearchable": true,  "bVisible": true,  "sWidth": "40px" ,
              "mRender": function ( source, type, val ) {
                  ...
                  // if (val[8] == 1 || val[8] == 2) {
                  if (val[8] == "Approve" || val[8] == "Tolak") {
                    disabled = 'disabled';
                  }
                  if(cek_aray == true){
                      checked = 'checked';
                  }
                  return '<input type="checkbox" value="'+val[1]+'"  name="chkbx" id="chkbx" class="form-check" vava="'+val[8]+'" onchange="update_sim(this.checked,\''+val[0]+'\')" '+checked+' '+disabled+'>';

                  // alert("val9 = " + val[9]+ ",," + "val0 = " + val[0]);
                  // alert(val[9]+" " +val[0]);
                  // console.log(val[9]);
                  }
            },
            ...
            { "bSearchable": false, "bVisible": true, "aTargets": [ 13 ], 
              "mRender": function( data, type, full) {
                ...
                if (full[12] == 1 || full[12] == 2 || full[12] == 3) {
                  var btn_a = '<a href="<?php echo URL_API_ANDROID?>pembatalan/'+full[13]+'" target="_blank" class="btn btn-info" title="Lihat Foto" style="margin-right: 5px"><i class="fa fa-file-image"></i></button>';
                } else {
                  var btn_a = '<a href="<?php echo URL_API_DISTRIBUSI?>gambar/spptbaru/'+full[14]+'" target="_blank" class="btn btn-info" title="Lihat Foto" style="margin-right: 5px"><i class="fa fa-file-image"></i></button>';
                }
                ...
                var btn_x = '<a href="<?php echo active_module_url()?>proses_laporan/detail/'+full[0]+'/?'+prm+'" target="_blank" class="btn btn-success" title="Detail" style="margin-right: 5px"><i class="fa fa-search"></i></button>';
                var btn_y = '<a href="<?php echo active_module_url()?>proses_laporan/terima/'+full[0]+'" class="btn btn-warning" title="Terima" style="margin-right: 5px"><i class="fa fa-check-circle"></i></button>';
                var btn_z = '<a href="<?php echo active_module_url()?>proses_laporan/tolak/'+full[0]+'" class="btn btn-danger" title="Tolak" ><i class="fa fa-times-circle"></i></button>';
          ...
          "bAutoWidth": false,
          "bServerSide": true,
          // "sAjaxSource": "<?php echo active_module_url();?>proses_laporan/grid/?sts=9"
      		"sAjaxSource": "<?php echo active_module_url();?>proses_laporan/grid/?"+data_params_awal
      	});
        ...
        // $('#gap_form').wrap('<form id="form_tarif" action="<?php echo active_module_url()?>proses_laporan/detail/" method="post"></form>');
        ...
        $("[id=btn_detail]").click(function(){
            if(ID){
                ...
                var data_params = decodeURIComponent($.param(params));
                window.location = '<?php echo active_module_url();?>proses_laporan/detail/'+ID+'/?'+data_params;

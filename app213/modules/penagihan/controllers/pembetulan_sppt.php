...
class pembetulan_sppt extends CI_Controller
{
    ...
    //     $this->load->model('MPembetulan_sppt');
    // }

    private $controller = 'pembetulan_sppt';
    public function __construct()
    {
        ...
        $this->load->model('pembetulan_sppt_model', 'MPembetulan_sppt');
    }
    ...
    public function index() {
        ...
        //------------------------------------------------------------------
        $select_data  = $this->MPembetulan_sppt->get_select_kecamatan();
    		$options      = array();
    		$kec_id = '';
    		if($select_data) {
        ...
        //------------------------------------------------------------------
        $kelurahan = $this->MPembetulan_sppt->get_select_kelurahan($kec_id);
    		$select_data = $this->MPembetulan_sppt->get_select_kelurahan($pawal_kec);
    		$options     = array();
        ...
        //------------------------------------------------------------------
        $select_data = $this->MPembetulan_sppt->get_select_id_piutang();
        ...
        $data['page_menu']  = 'pembetulan_sppt';
        $data['current']    = '';
        $data['controller'] = $this->controller;
        $data['apps']       = $this->apps_model->get_active_only();

        $this->load->view('v_pembetulan_sppt', $data);
    }
    ...
    function grid()
    {
        ...
        // echo $this->MPembetulan_sppt->getUserds();
    ...
    private function fpost() {
        ...
        $data[LUAS_BUMI_OLD_DSP] = $this->input->post('luas_bumi_op_a');
        $data[LUAS_BNG_OLD_DSP] = $this->input->post('luas_bng_a');
        $data[LUAS_BUMI_NEW_DSP] = $this->input->post('luas_bumi_op_b');
        $data[LUAS_BNG_NEW_DSP] = $this->input->post('luas_bng_b');
    ...
    public function detail(){
        ...
        $data['faction'] = active_module_url("pembetulan_sppt/approve/{$p_id}");

        if ($p_id && $get = $this->MPembetulan_sppt->get($p_id)) {
          ...
          $data['dt'][JALAN_WP_NEW_DSP] = empty($get->JALAN_WP_NEW) ? NULL : $get->JALAN_WP_NEW;
          $data['dt'][LUAS_BUMI_OLD_DSP] = empty($get->LUAS_BUMI_OLD) ? NULL : $get->LUAS_BUMI_OLD;
          $data['dt'][LUAS_BNG_OLD_DSP] = empty($get->LUAS_BNG_OLD) ? NULL : $get->LUAS_BNG_OLD;
          $data['dt'][LUAS_BUMI_NEW_DSP] = empty($get->LUAS_BUMI_NEW) ? NULL : $get->LUAS_BUMI_NEW;
          $data['dt'][LUAS_BNG_NEW_DSP] = empty($get->LUAS_BNG_NEW) ? NULL : $get->LUAS_BNG_NEW;
          $data['dt'][STATUS_DSP] = empty($get->STATUS) ? NULL : $get->STATUS;
          $data['dt'][NOP_DSP] = empty($get->NOP) ? NULL : $get->NOP;
          $data['dt'][THN_PAJAK_SPPT_DSP] = empty($get->THN_PAJAK_SPPT) ? NULL : $get->THN_PAJAK_SPPT;

          $nop = $data['dt'][NOP_DSP];
          $thn = $data['dt'][THN_PAJAK_SPPT_DSP];

          if($nop){
            $gmbr = $this->MPembetulan_sppt->gambar($nop, $thn);
            $cek_gmbr = $this->MPembetulan_sppt->load_gambar($nop, $thn);
            // var_dump($cek_gmbr->FOTO_PEMBETULAN);die;
            $data['dt'][GMBRTT12] = $gmbr;
            $data['dt']['gmbr'] = empty($cek_gmbr->FOTO_PEMBETULAN) ? NULL : $cek_gmbr->FOTO_PEMBETULAN;
            // $data['dt'][GMBRTT12] = empty($gmbr->FOTO_PEMBETULAN) ? NULL : 1;
          }
          ...
          $this->load->view('v_pembetulan_sppt_form', $data);
          // $this->load->view('v_pembetulan_sppt', $data);
        } else {
          show_404();
        }
    ...
    public function approve(){
        ...
        if ($this->form_validation->run() == TRUE) {
          if ($p_id && $get = $this->MPembetulan_sppt->get($p_id)) {
            ...
            $this->MPembetulan_sppt->update_dsp($nop, $update_dsp);
            // UPDATE DAT_OBJEK_PAJAK
            $this->MPembetulan_sppt->update_dop($nop, $update_dop);
            ...
            // $this->MPembetulan_sppt->update($p_id, $update_data);
            $this->MPembetulan_sppt->update_manuwal($p_id);
    ...
    function process_tolak(){
        ...
        if($id && $get = $this->MPembetulan_sppt->get($id)){
            $this->MPembetulan_sppt->process_tolak($id);

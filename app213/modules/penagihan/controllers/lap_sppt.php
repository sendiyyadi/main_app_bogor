...
class lap_sppt extends CI_Controller
{
    private $controller = 'lap_sppt';
    public function __construct()
    {
        ...
        $this->load->model('lap_sppt_model', 'Mlap_sppt');
    }
    ...
    public function index() {
        ...
        //------------------------------------------------------------------
        $select_data  = $this->Mlap_sppt->get_select_kecamatan();
    		$options      = array();
    		$kec_id = '';
    		if($select_data) {
        ...
        //------------------------------------------------------------------
        $kelurahan = $this->Mlap_sppt->get_select_kelurahan($kec_id);
    		$select_data = $this->Mlap_sppt->get_select_kelurahan($pawal_kec);
    		$options     = array();
        ...
        $data['page_menu']  = 'lap_sppt';
        $data['current']    = '';
        $data['controller'] = $this->controller;
        $data['apps']       = $this->apps_model->get_active_only();

        $this->load->view('v_lap_sppt', $data);
    }
    ...
    function grid()
    {
        ...
        $this->datatables->where('T1.ID_PIUTANG = 1');
    ...
    public function terima() {
      ...
      if ($p_id && $get = $this->Mlap_sppt->get_dt_komplit_by_nopthn($p_id)) {
        $prs = $this->Mlap_sppt->proses_terima($p_id);
        if ($prs) {
  ...
  public function tolak() {
    ...
    if ($p_id && $get = $this->Mlap_sppt->get_dt_komplit_by_nopthn($p_id)) {
      $prs = $this->Mlap_sppt->proses_tolak($p_id);
      if ($prs) {
  ...
  public function terima_sim() {
        ...
        for ($i=0; $i < count($nop); $i++) { 
            $nop_id = $nop[$i];
            // echo $nop_id; die();
            if ($nop_id && $data = $this->Mlap_sppt->get_by_nopthn($nop_id)) {
                // echo $data->STSVER; die();
                if ($data->STSVER == '0') {
                    $this->Mlap_sppt->proses_terima($nop_id);
                } else {
                    $not_sts4 += 1;
                }
            } else {
  ...
  public function tolak_sim() {
      ...
      for ($i=0; $i < count($nop); $i++) { 
          ...
          if ($nop_id && $data = $this->Mlap_sppt->get_by_nopthn($nop_id)) {
          if ($data->STSVER == '0') {
              $this->Mlap_sppt->proses_tolak($nop_id);
          } else {
              $not_sts4 += 1;
          }
      } else {
  ...
  public function detail(){
      ...
      if ($p_id && $get = $this->Mlap_sppt->get_dt_komplit_by_nopthn_real($p_id)) {
        ...
        $data['dt']['tlp'] = empty($get->TELP) ? NULL : $get->TELP;
        $data['dt']['email'] = empty($get->EMAIL) ? NULL : $get->EMAIL;
        $data['dt']['golongan_listrik'] = empty($get->NM_LOOKUP_ITEM) ? NULL : $get->NM_LOOKUP_ITEM;
        $data['dt']['batasan_daya'] = empty($get->NM_REF_LISTRIK) ? NULL : $get->NM_REF_LISTRIK;
        $data['dt']['plat_2'] = empty($get->PLAT_2) ? NULL : $get->PLAT_2;
        $data['dt']['plat_4'] = empty($get->PLAT_4) ? NULL : $get->PLAT_4;
        $data['dt']['nama_penerima'] = empty($get->NAMA_PENERIMA_SPPT) ? NULL : $get->NAMA_PENERIMA_SPPT;
        $data['dt']['sts_penerima'] = empty($get->STATUS_PENERIMA) ? NULL : $get->STATUS_PENERIMA;

        // if ($get->STATUS_BATAL_NOP == 1 ){
        if ($get->STATUS_BATAL_NOP == 1 || $get->STATUS_BATAL_NOP == 2 || $get->STATUS_BATAL_NOP == 3){
          $get_btl = $this->Mlap_sppt->get_dt_btl($p_id);
        ...
        // var_dump($get->STATUS_BATAL_NOP);die;
        // var_dump($file);die;
        ...
        if (empty($file)) {
          ...
          if ($get->STATUS_BATAL_NOP == 1 || $get->STATUS_BATAL_NOP == 2 || $get->STATUS_BATAL_NOP == 3){
            ...
            $cek_url = URL_API_SPPT_NEO_R.'gambar/spptbaru/'.$file;
        ...
        $this->load->view('v_lap_sppt_form', $data);
        // $this->load->view('v_lap_sppt', $data);
      } else {
        show_404();
      }
  ...
  public function exp_excel_csv(){
    ...
    $query = $this->Mlap_sppt->query_cetak_real($kd_kec, $kd_kel, $c_thn, $c_nop);

...
class pdl_pembetulan_kabid extends CI_Controller {
    ...
    public function index() {
        ...
        $data['current'] = 'pdl_pembetulan_kabid';
        ...
        $this->load->view('vpdl_pembetulan_kabid', $data);
    }
    ...
    public function grid() {
        ...
        $this->datatables->select("	P.ID, P.THN_PELAYANAN||'-'||P.BUNDEL_PELAYANAN||'-'||P.NO_URUT_PELAYANAN as NOPEL,
                                    TO_CHAR(P.TGL_SUBID_PDL,'DD-MM-YYYY') as TGL_APR,
        ...
        // $this->datatables->where("P.STATUS_PERMOHONAN IN ('7')");

        if($sts_kd <> '888' && !empty($sts_kd)){
            $this->datatables->where("P.STATUS_PERMOHONAN NOT IN ('0','1','2','3','4','6','7')");
        ...
        if(!is_super_admin()) {
            $this->datatables->where("P.NIP_BID_PDL", $nip_login);
        }
        ...
        if(!empty($tgl_fr)){
            $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_SUBID_PDL,'yyyy-mm-dd'), 'yyyy-mm-dd') >= TO_DATE('".$tgl_fr."', 'dd-mm-yyyy')");
        }

        if(!empty($tgl_to)){
            $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_SUBID_PDL,'yyyy-mm-dd'), 'yyyy-mm-dd') <= TO_DATE('".$tgl_to."', 'dd-mm-yyyy')");
        }
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/pdl_pembetulan_kabid');
        // }
        ...
        if ($dt){
            ...
            $this->load->view('vpdl_pembetulan_kabid_form', $data);

        } else {
            $this->session->set_flashdata('msg_danger', 'Data tidak ditemukan..');
            redirect(active_module_url('pdl_pembetulan_kabid'));
        }
    ...
    public function edit() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN <> '7'";
        $cek_heula = $this->db->query($qr_cek, [$param])->row()->JML;
        ...
        if ($dt){
            ...
            if ($dt->STATUS_PERMOHONAN != '7') {
                $this->session->set_flashdata('msg_danger', 'Tidak bisa edit data.');
                redirect(active_module_url('pdl_pembetulan_kabid'));
            }
    ...
    function approve_permohonan() {
        ...
        if ($dt){
            ...
            if (!empty($err_Msg_2)) {
                ...
                $res_msg    = 'Data berhasil diapprove Kabid Pendanil';
                $res_code   = 200;
            }
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'pdl_pembetulan_kabid/openblob_permo/' . $field . '/' . $id_ppo;

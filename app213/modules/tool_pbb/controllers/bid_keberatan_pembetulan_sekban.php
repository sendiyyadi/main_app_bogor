...
class bid_keberatan_pembetulan_sekban extends CI_Controller {
    public function __construct() {
        ...
        $module = 'bid_keberatan_pembetulan_sekban';
    ...
    public function index() {
        ...
        $data['current'] = 'bid_keberatan_pembetulan_sekban';
        ...
        $this->load->view('vbid_keberatan_pembetulan_sekban', $data);
    }
    ...
    public function grid() {
        ...
        if(!is_super_admin()) {
            $this->datatables->where("P.NIP_SEKBAN", $nip_login);
        }
        ...
        if(!empty($sts_kd)){
            if($sts_kd == '1'){
                $this->datatables->where("P.STATUS_PERMOHONAN IN ('J')");
            } else {
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/bid_keberatan_pembetulan_sekban');
        // }
        ...
        if ($dt){
            ...
            $this->load->view('vbid_keberatan_pembetulan_sekban_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('bid_keberatan_pembetulan_sekban'));
        }
    ...
    public function edit() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN not in ('J')";
        $cek_heula = $this->db->query($qr_cek, [$param])->row()->JML;
    ...
    function approve_permohonan() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN <> 'J'";
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'bid_keberatan_pembetulan_sekban/openblob_permo/' . $field . '/' . $id_ppo;

...
class pnt_pengurangan_verif extends CI_Controller {
    ...
    public function index() {
        ...
        $data['current'] = 'pnt_pengurangan_verif';
        ...
        $this->load->view('vpnt_pengurangan_verif', $data);
    }
    ...
    public function grid() {
        ...
        $this->datatables->select("	P.ID, P.THN_PELAYANAN||'-'||P.BUNDEL_PELAYANAN||'-'||P.NO_URUT_PELAYANAN as NOPEL,
                                    TO_CHAR(P.TGL_BID_PKP,'DD-MM-YYYY') as TGL_APR,
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/pnt_pengurangan_verif');
        // }
        ...
        if ($x){
            ...
            $this->load->view('vpnt_pengurangan_verif_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('pnt_pengurangan_verif'));
        }
    ...
    public function edit() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN not in ('H')";
        $cek_heula = $this->db->query($qr_cek, [$param])->row()->JML;
    ...
    function approve_permohonan() {
        ...
        if ($dt) {
            ...
            $sql .= " BEGIN
                        UPDATE PST_PERMOHONAN_TOOL SET STATUS_PERMOHONAN = 'B', TGL_VER_PNTP = SYSTIMESTAMP
                        WHERE ID =  {$id_ppo} ;
            ...
            // $err_Msg_2 = null;
            if (!empty($err_Msg_2)) {
                $error_CRUD = $err_Msg_2 . ', Proses Approve Verifikasi Penetapan gagal....!!!';
                $res_msg    = $error_CRUD;
                $res_code   = 500;
            } else {
                $res_msg    = 'Data berhasil diapprove verifikasi penetapan';
                $res_code   = 200;
            }
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'pnt_pengurangan_verif/openblob_permo/' . $field . '/' . $id_ppo;

...
class bid_keberatan_pembetulan_verif extends CI_Controller {
    public function __construct() {
        ...
        $module = 'bid_keberatan_pembetulan_verif';
    ...
    public function index() {
        ...
        $data['current'] = 'bid_keberatan_pembetulan_verif';
        ...
        $this->load->view('vbid_keberatan_pembetulan_verif', $data);
    }
    ...
    public function grid() {
        ...
        $this->datatables->select("	P.ID, P.THN_PELAYANAN||'-'||P.BUNDEL_PELAYANAN||'-'||P.NO_URUT_PELAYANAN as NOPEL,
                                    CASE WHEN P.TGL_BID_PDL IS NULL THEN TO_CHAR(P.TGL_APR_LOKET,'DD-MM-YYYY') 
                                        ELSE TO_CHAR(P.TGL_BID_PDL,'DD-MM-YYYY') end as TGL_APR,
        ...
        // if(!empty($tgl_fr)){
        //     $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_BID_PDL,'yyyy-mm-dd'), 'yyyy-mm-dd') >= TO_DATE('".$tgl_fr."', 'dd-mm-yyyy')");
        // }

        // if(!empty($tgl_to)){
        //     $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_BID_PDL,'yyyy-mm-dd'), 'yyyy-mm-dd') <= TO_DATE('".$tgl_to."', 'dd-mm-yyyy')");
        // }

        //// filter pake yg terbaru tgl_bid_pdl atau tgl_loket
        $col_loket = "TO_DATE(TO_CHAR(P.TGL_APR_LOKET,'yyyy-mm-dd'), 'yyyy-mm-dd')";
        $col_pdl   = "TO_DATE(TO_CHAR(P.TGL_BID_PDL,'yyyy-mm-dd'), 'yyyy-mm-dd')";

        if (!empty($tgl_fr) && !empty($tgl_to)) {
            // Kalau $tgl_fr dan $tgl_to dua-duanya ada nilainya (Filter Range Tanggal)
            $format_fr = "TO_DATE('".$tgl_fr."', 'dd-mm-yyyy')";
            $format_to = "TO_DATE('".$tgl_to."', 'dd-mm-yyyy')";
            
            $this->datatables->where("(
                ($col_loket >= $format_fr AND $col_loket <= $format_to) OR 
                ($col_pdl >= $format_fr AND $col_pdl <= $format_to)
            )", NULL, FALSE); 
            
        } elseif (!empty($tgl_fr)) {
            // Kalau cuma isi $tgl_fr (Mulai dari tanggal sekian)
            $format_fr = "TO_DATE('".$tgl_fr."', 'dd-mm-yyyy')";
            
            $this->datatables->where("(
                $col_loket >= $format_fr OR 
                $col_pdl >= $format_fr
            )", NULL, FALSE);

        } elseif (!empty($tgl_to)) {
            // Kalau cuma isi $tgl_to (Sampai tanggal sekian)
            $format_to = "TO_DATE('".$tgl_to."', 'dd-mm-yyyy')";
            
            $this->datatables->where("(
                $col_loket <= $format_to OR 
                $col_pdl <= $format_to 
            )", NULL, FALSE);
        }
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/bid_keberatan_pembetulan_verif');
        // }
        ...
        if ($dt){
            ...
            $this->load->view('vbid_keberatan_pembetulan_verif_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('bid_keberatan_pembetulan_verif'));
        }
    ...
    function approve_permohonan() {
        ...
        if ($dt){
            ...
            //// Cek Sub Jenis Pelayanan == 02 -> insert baru lampiran
            // if ($dt->KD_SUB_JNS_PELAYANAN == '02') {
                $fim_l_pkp1 = empty($_FILES['im_l_pkp1']['name']) ? 0 : 1;
                $fim_l_pkp2 = empty($_FILES['im_l_pkp2']['name']) ? 0 : 1;

                $im_l_pkp1 = $fim_l_pkp1 == 0 ? NULL : file_get_contents($_FILES['im_l_pkp1']['tmp_name']);
                $im_l_pkp2 = $fim_l_pkp2 == 0 ? NULL : file_get_contents($_FILES['im_l_pkp2']['tmp_name']);

                if (!empty($_FILES['im_l_pkp1']['name']) || !empty($_FILES['im_l_pkp2']['name']) ) {
                    $this->update_prm_blob($id_ppo);
                }
            // }

            $add_dt_pst = array();
            //// cek THN_SK udah ada belum (kalo lewat pendanil, udah ada thn sk), kalo data langsung dari loket ga ada thn sk (generate dari awal)
            if ($dt->THN_SK) {
            ...
            // // $ins_no_sk = '973/' . str_pad($new_urut_sk, 6, "0", STR_PAD_LEFT) . '/PD-II/SK/PembetulanIntenEksten/' . bulan_romawi(date('m')) . '/' . $new_thn_sk;
            // // $ins_no_lhp = '973/' . str_pad($new_urut_sk, 6, "0", STR_PAD_LEFT) . '/PD-II/LHP/PembetulanPBBIntenEksten/' . bulan_romawi(date('m')) . '/' . $new_thn_sk;
            // $ins_no_bap = '000.1.6/' . str_pad($new_urut_sk, 6, "0", STR_PAD_LEFT) . '/PEMBETULAN-PKP/' . bulan_romawi(date('m')) . '/' . $new_thn_sk;

            // $tgl_sk = empty($tgl_sk) ? NULL : date('Y-m-d', strtotime($tgl_sk));
            // $tgl_lhp = empty($tgl_lhp) ? NULL : date('Y-m-d', strtotime($tgl_lhp));
            // $tgl_bap = empty($tgl_bap) ? NULL : date('Y-m-d', strtotime($tgl_bap));
            $tgl_bap = date('Y-m-d');
            ...
            $dt_pst = array(
                ...
                'KET_VERLAP'    => $this->input->post('ket_verlap'),
                'REKOM_VERLAP'  => $this->input->post('rekom_verlap'),
                'PROSES_PK'     => $this->input->post('proses_pk'),
                'ANALISA_PK'    => $this->input->post('analisa_pk'),
                'KETERANGAN_PK' => $this->input->post('keterangan_pk'),
                'PROSES_PL'     => $this->input->post('proses_pl'),
                'ANALISA_PL'    => $this->input->post('analisa_pl'),
                'KETERANGAN_PL' => $this->input->post('keterangan_pl')
            );

            $dt_pst = array_merge($dt_pst, $add_dt_pst);

            // //// tambahan ketika sub jns 02 
            // if ($dt->KD_SUB_JNS_PELAYANAN == '02') {
            ...
            // $err_Msg_2 = null;
            if (!empty($err_Msg_2)) {
                $error_CRUD = $err_Msg_2 . ', Proses Approve Verifikator Bidang Keberatan gagal....!!!';
                $res_msg    = $error_CRUD;
                $res_code   = 500;
            } else {
                $res_msg    = 'Data berhasil diapprove verifikator bidang keberatan';
                $res_code   = 200;
            }
    ...
    function update_prm_blob($id_ppo) { 
        ...
        if (!empty($_FILES['im_l_pkp1']['name'])) {
            array_push($tbl_field, 'L_PKP11=EMPTY_BLOB()');
            array_push($fl_blob, 'L_PKP1=1');
            array_push($tbl_field_return, 'L_PKP11');
            array_push($return_blob, ':blob3');
            $im_l_pkp1 = file_get_contents($_FILES['im_l_pkp1']['tmp_name']);
        }
        if (!empty($_FILES['im_l_pkp2']['name'])) {
            array_push($tbl_field, 'L_PKP21=EMPTY_BLOB()');
            array_push($fl_blob, 'L_PKP2=1');
            array_push($tbl_field_return, 'L_PKP21');
            array_push($return_blob, ':blob4');
            $im_l_pkp2 = file_get_contents($_FILES['im_l_pkp2']['tmp_name']);
        }
        ...
        if (!empty($_FILES['im_l_pkp1']['name'])) {
            $blob3->save($im_l_pkp1);
        }
        if (!empty($_FILES['im_l_pkp2']['name'])) {
            $blob4->save($im_l_pkp2);
        }
        // if (!empty($_FILES['im_l_verlap3']['name'])) {
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'bid_keberatan_pembetulan_verif/openblob_permo/' . $field . '/' . $id_ppo;

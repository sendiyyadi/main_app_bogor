...
class bid_keberatan_pembetulan extends CI_Controller {
    public function __construct() {
        ...
        $module = 'bid_keberatan_pembetulan';
    ...
    public function index() {
        ...
        $data['current'] = 'bid_keberatan_pembetulan';
        $data['apps']    = $this->apps_model->get_active_only();
        $option = array( 
            '1' => 'Belum Proses',
            '8' => 'Approve Koordinator PKP',
            'D' => 'Tolak PKP',);
        ...
        $this->load->view('vbid_keberatan_pembetulan', $data);
    }
    ...
    public function grid() {
        ...
        $this->datatables->select("	P.ID, P.THN_PELAYANAN||'-'||P.BUNDEL_PELAYANAN||'-'||P.NO_URUT_PELAYANAN as NOPEL,
                                    TO_CHAR(P.TGL_BID_PDL,'DD-MM-YYYY') as TGL_APR,
        ...
        if(!is_super_admin()) {
            $this->datatables->where("P.NIP_KOOR_PKP", $nip_login);
        }
        ...
        if(!empty($tgl_fr)){
            $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_VER_PKP,'yyyy-mm-dd'), 'yyyy-mm-dd') >= TO_DATE('".$tgl_fr."', 'dd-mm-yyyy')");
        }

        if(!empty($tgl_to)){
            $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_VER_PKP,'yyyy-mm-dd'), 'yyyy-mm-dd') <= TO_DATE('".$tgl_to."', 'dd-mm-yyyy')");
        }

        if(!empty($sts_kd)){
            if($sts_kd == '1'){
                $this->datatables->where("P.STATUS_PERMOHONAN IN ('A')");
            } else {
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/bid_keberatan_pembetulan');
        // }
        ...
        if ($dt){
            ...
            $this->load->view('vbid_keberatan_pembetulan_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('bid_keberatan_pembetulan'));
        }
    ...
    public function edit() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN not in ('A')";
        $cek_heula = $this->db->query($qr_cek, [$param])->row()->JML;
    ...
    function approve_permohonan() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN <> 'A'";
        ...
        if ($dt){
            ...
            // $err_Msg_2 = null;
            if (!empty($err_Msg_2)) {
                $error_CRUD = $err_Msg_2 . ', Proses Approve Koordinator Bidang Keberatan gagal....!!!';
                $res_msg    = $error_CRUD;
                $res_code   = 500;
            } else {
                $res_msg    = 'Data berhasil diapprove koordinator bidang keberatan';
                $res_code   = 200;
            }
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'bid_keberatan_pembetulan/openblob_permo/' . $field . '/' . $id_ppo;

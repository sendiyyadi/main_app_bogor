...
class bid_keberatan_pembetulan_kaban extends CI_Controller {
    public function __construct() {
        ...
        $module = 'bid_keberatan_pembetulan_kaban';
    ...
    public function index() {
        ...
        $data['current'] = 'bid_keberatan_pembetulan_kaban';
        ...
        $this->load->view('vbid_keberatan_pembetulan_kaban', $data);
    }
    ...
    public function grid() {
        ...
        if(!is_super_admin()) {
            $this->datatables->where("P.NIP_KABAN", $nip_login);
        }
        ...
        if(!empty($sts_kd)){
            if($sts_kd == '1'){
                $this->datatables->where("P.STATUS_PERMOHONAN IN ('I')");
            } else {
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/bid_keberatan_pembetulan_kaban');
        // }
        ...
        if ($dt){
            ...
            $this->load->view('vbid_keberatan_pembetulan_kaban_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('bid_keberatan_pembetulan_kaban'));
        }
    ...
    public function edit() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN not in ('I')";
        $cek_heula = $this->db->query($qr_cek, [$param])->row()->JML;
    ...
    function approve_permohonan() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN <> 'I'";
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'bid_keberatan_pembetulan_kaban/openblob_permo/' . $field . '/' . $id_ppo;

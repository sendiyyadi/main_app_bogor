...
class pdl_mutasi_habis extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('vpdl_mutasi_habis', $data);
    }
    ...
    public function detail() {
        ...
        if ($dt) {
            ...
            $this->load->view('vpdl_mutasi_habis_form', $data);

        } else {
            $this->session->set_flashdata('msg_success', 'Data tidak ditemukan..');
            redirect(active_module_url('pdl_mutasi_habis'));
        }
    ...
    public function edit() {
        ...
        if ($cek_heula > 0) {
            $this->session->set_flashdata('msg_info', "Data sudah diapprove. Tidak bisa edit data");
            redirect('tool_pbb/pdl_mutasi_habis');
        }
    ...
    function approve_permohonan() {
        ...
        if ($dt){
            ...
            $error_1 = $this->permohonan_online_upt_model->update_pst_permohonan_tool($id_ppo, $dt_pst);

            //// update status ke pegawai baru
            $this->permohonan_online_upt_model->set_next_pegawai($id_next_pgw, $jns_bid, $kdjns, $kdsub);

            //// update tracking
            $sql_tracking = "UPDATE TRACKING SET TGL_VER_KEB=SYSDATE, KET_ARSIP=(SELECT NAMA FROM CLAPPPDN WHERE KODE='T'), 
    ...
    public function tolak_permohonan() { 
        ...
        if ($p_id && $get = $this->permohonan_online_upt_model->get_ppo_by_id($p_id)) {
            if (intval($get->STATUS_PERMOHONAN) == '4') {
                ...
                if (!empty($err_Msg_2)) {
                    ...
                    if ($this->email->send()) {
                        ...
                        // redirect(active_module_url('loket_permohonan_online_upt'));
                        echo 'Data berhasil di tolak (Gagal Kirim Email)';
                    }
                    
                 }
             } else {
                echo 'ERROR... Status Dokumen Bukan Verifikasi';
             }
    ...
    public function tolak_ke_pelayanan() { 
        $p_id               = $this->input->post('id_ppo');
        $ket_tolak_ply      = strtoupper($this->input->post('ket_tolak_ply'));

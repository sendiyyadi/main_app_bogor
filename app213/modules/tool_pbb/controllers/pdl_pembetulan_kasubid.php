...
class pdl_pembetulan_kasubid extends CI_Controller {
    ...
    public function index() {
        ...
        $data['current'] = 'pdl_pembetulan_kasubid';
        $data['apps']    = $this->apps_model->get_active_only();
        $option = array( ''=> 'Semua Status',
            '888' => 'Belum diproses',
            '999' => 'Sudah diproses',
            'C' => 'Permohonan Tidak Dikabulkan');
        ...
        $this->load->view('vpdl_pembetulan_kasubid', $data);
    }
    ...
    public function grid() {
        ...
        $this->datatables->select(" P.ID, P.THN_PELAYANAN||'-'||P.BUNDEL_PELAYANAN||'-'||P.NO_URUT_PELAYANAN as NOPEL,
                                    TO_CHAR(P.TGL_VER_PDL,'DD-MM-YYYY') as TGL_APR,
        ...
        // $this->datatables->where("P.STATUS_PERMOHONAN IN ('6')");

        if($sts_kd <> '888' && !empty($sts_kd)){
            if($sts_kd == 'C') {
                $this->datatables->where("P.STATUS_PERMOHONAN IN ('C')");
            } else {
                $this->datatables->where("P.STATUS_PERMOHONAN NOT IN ('0','1','2','3','4','6')");
        ...
        if(!is_super_admin()) {
            $this->datatables->where("P.NIP_SUBID_PDL", $nip_login);
        }
        ...
        if(!empty($tgl_fr)){
            $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_VER_PDL,'yyyy-mm-dd'), 'yyyy-mm-dd') >= TO_DATE('".$tgl_fr."', 'dd-mm-yyyy')");
        }

        if(!empty($tgl_to)){
            $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_VER_PDL,'yyyy-mm-dd'), 'yyyy-mm-dd') <= TO_DATE('".$tgl_to."', 'dd-mm-yyyy')");
        }
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/pdl_pembetulan_kasubid');
        // }
        ...
        if ($dt){
            ...
            $this->load->view('vpdl_pembetulan_kasubid_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('pdl_pembetulan_kasubid'));
        }
    ...
    public function edit() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN NOT IN ('6', 'C')";
        $cek_heula = $this->db->query($qr_cek, [$param])->row()->JML;
        ...
        if ($dt){
            ...
            if (!in_array($dt->STATUS_PERMOHONAN, ['6', 'C'])) {
    ...
    function approve_permohonan() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN <> '6'";
        ...
        if ($dt){
            ...
            $dt_pst = array(
                ...
                'STATUS_PERMOHONAN' => '7',
                'NIP_BID_PDL'       => $nip_next_pgw,
                'TGL_SUBID_PDL'     => current_time_ora(),
            );
            ...
            // $err_Msg_2 = null;
            if (!empty($err_Msg_2)) {
                $error_CRUD = $err_Msg_2 . ', Proses Approve Kasubid Pendanil gagal....!!!';
                $res_msg    = $error_CRUD;
                $res_code   = 500;
            } else {
                $res_msg    = 'Data berhasil diapprove Kasubid Pendanil';
                $res_code   = 200;
            }
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'pdl_pembetulan_kasubid/openblob_permo/' . $field . '/' . $id_ppo;

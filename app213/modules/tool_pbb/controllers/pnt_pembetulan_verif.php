...
class pnt_pembetulan_verif extends CI_Controller {
    ...
    public function index() {
        ...
        $data['current'] = 'pnt_pembetulan_verif';
        ...
        $this->load->view('vpnt_pembetulan_verif', $data);
    }
    ...
    public function grid() {
        ...
        $this->datatables->select(" P.ID, P.THN_PELAYANAN||'-'||P.BUNDEL_PELAYANAN||'-'||P.NO_URUT_PELAYANAN as NOPEL,
                                    CASE WHEN P.KD_SUB_JNS_PELAYANAN = '03' THEN TO_CHAR(P.TGL_APR_LOKET,'DD-MM-YYYY') ELSE
                                        ( CASE WHEN P.TGL_KABAN IS NOT NULL THEN TO_CHAR(P.TGL_KABAN,'DD-MM-YYYY')
                                        ELSE TO_CHAR(P.TGL_BID_PKP,'DD-MM-YYYY') END ) 
                                    END as TGL_APR,
        ...
        if(!is_super_admin()) {
            $this->datatables->where("P.NIP_VER_PNTP", $nip_login);
        }
        ...
        // Kita bikin alias string dulu biar syntax datatables-nya nggak kepanjangan
        $col_loket = "TO_DATE(TO_CHAR(P.TGL_APR_LOKET,'yyyy-mm-dd'), 'yyyy-mm-dd')";
        $col_pkp   = "TO_DATE(TO_CHAR(P.TGL_BID_PKP,'yyyy-mm-dd'), 'yyyy-mm-dd')";
        $col_kaban = "TO_DATE(TO_CHAR(P.TGL_KABAN,'yyyy-mm-dd'), 'yyyy-mm-dd')";
        ...
        if (!empty($tgl_fr) && !empty($tgl_to)) {
            ...
            $this->datatables->where("(
                ...
                ($col_pkp >= $format_fr AND $col_pkp <= $format_to) OR 
                ($col_kaban >= $format_fr AND $col_kaban <= $format_to)
            )", NULL, FALSE); 
        ...
        if(!empty($sts_kd)){
            ...
            $this->datatables->where("P.STATUS_PERMOHONAN IN ('H')");
        }
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/pnt_pembetulan_verif');
        // }
        ...
        if ($dt){
            ...
            $this->load->view('vpnt_pembetulan_verif_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('pnt_pembetulan_verif'));
        }
    ...
    public function edit() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN <> 'H'";
        $cek_heula = $this->db->query($qr_cek, [$param])->row()->JML;
    ...
    function approve_permohonan() {
        ...
        if ($dt){
            ...
            $dt_pst = array(
                'STATUS_PERMOHONAN' => 'B',
                'TGL_VER_PNTP'      => current_time_ora(),
            );

            //// tambahan ketika sub jns 03 
            if ($dt->KD_SUB_JNS_PELAYANAN == '03') {
            ...
            $res_msg    = 'Data berhasil diproses Verifikator Penetapan';
            $res_code   = 200;
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'pnt_pembetulan_verif/openblob_permo/' . $field . '/' . $id_ppo;

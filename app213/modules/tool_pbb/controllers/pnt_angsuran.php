...
class pnt_angsuran extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('vpnt_angsuran', $data);
    }
    ...
    public function grid() {
        ...
        $this->datatables->where("P.KD_JNS_PELAYANAN", "15");
        ...
        if(!empty($sts_kd)){
            if($sts_kd == '1'){
                $this->datatables->where("P.STATUS_PERMOHONAN IN ('A', 'C')");
            } else {
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/pnt_angsuran');
        // }
        ...
        if ($x){
            ...
            $this->load->view('vpnt_angsuran_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('pnt_angsuran'));
        }
    ...
    function approve_permohonan() {
        ...
        if ($dt) {
            ...
            $sql .= " BEGIN
                        UPDATE PST_PERMOHONAN_TOOL SET STATUS_PERMOHONAN = '5'
                        WHERE ID =  {$id_ppo} ;
            ...
            //// UPDATE SPPT
            $sql .= " BEGIN
                        UPDATE SPPT SET STATUS_PEMBAYARAN_SPPT = '9'
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'pnt_angsuran/openblob_permo/' . $field . '/' . $id_ppo;

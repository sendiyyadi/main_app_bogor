...
class pnt_aktivasi_nop extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('vpnt_aktivasi_nop', $data);
    }
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/pnt_aktivasi_nop');
        // }
        ...
        if ($x){
            ...
            $this->load->view('vpnt_aktivasi_nop_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('pnt_aktivasi_nop'));
        }
    ...
    function approve_permohonan() {
        ...
        if ($dt) {
            ...
            $sql = "CALL SP_PNT_AKTIVASI_OL({$id_ppo}, '{$nip_pencetak}') ";
            ...
            // $err_Msg = null;
            if (!empty($err_Msg)) {
                $error_CRUD = $err_Msg . ', Proses Approve Penetapan gagal....!!!';
                $res_msg    = $error_CRUD;
                $res_code   = 500;
            } else {
                //// update tracking
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'pnt_aktivasi_nop/openblob_permo/' . $field . '/' . $id_ppo;

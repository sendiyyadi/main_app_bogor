...
class bid_keberatan_pembetulan_kabid extends CI_Controller {
    public function __construct() {
        ...
        $module = 'bid_keberatan_pembetulan_kabid';
    ...
    public function index() {
        ...
        $data['current'] = 'bid_keberatan_pembetulan_kabid';
        ...
        $this->load->view('vbid_keberatan_pembetulan_kabid', $data);
    }
    ...
    public function grid() {
        ...
        $this->datatables->select("	P.ID, P.THN_PELAYANAN||'-'||P.BUNDEL_PELAYANAN||'-'||P.NO_URUT_PELAYANAN as NOPEL,
                                    TO_CHAR(P.TGL_SUBID_PKP,'DD-MM-YYYY') as TGL_APR,
        ...
        if(!is_super_admin()) {
            $this->datatables->where("P.NIP_BID_PKP", $nip_login);
        }
        ...
        if(!empty($tgl_fr)){
            $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_SUBID_PKP,'yyyy-mm-dd'), 'yyyy-mm-dd') >= TO_DATE('".$tgl_fr."', 'dd-mm-yyyy')");
        }

        if(!empty($tgl_to)){
            $this->datatables->where("TO_DATE(TO_CHAR(P.TGL_SUBID_PKP,'yyyy-mm-dd'), 'yyyy-mm-dd') <= TO_DATE('".$tgl_to."', 'dd-mm-yyyy')");
        }

        if(!empty($sts_kd)){
            if($sts_kd == '1'){
                $this->datatables->where("P.STATUS_PERMOHONAN IN ('G')");
            } else {
    ...
    public function detail() {
        ...
        //     redirect('tool_pbb/bid_keberatan_pembetulan_kabid');
        // }
        ...
        if ($dt){
            ...
            $this->load->view('vbid_keberatan_pembetulan_kabid_form', $data);

        } else {
            $this->session->set_flashdata('msg_error', 'Data tidak ditemukan..');
            redirect(active_module_url('bid_keberatan_pembetulan_kabid'));
        }
    ...
    public function edit() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN not in ('G')";
        $cek_heula = $this->db->query($qr_cek, [$param])->row()->JML;
    ...
    function approve_permohonan() {
        ...
        //// cek heula
        $qr_cek = "SELECT COUNT(*) AS JML FROM PST_PERMOHONAN_TOOL WHERE ID = ? AND STATUS_PERMOHONAN <> 'G'";
        ...
        if ($dt){
            ...
            //// disable, sekarang cek buku 5 -> sekban -> kaban -> pnt, selain itu langsung ke penetapan
            // $jns_bid = '3B';
            ...
            // $err_Msg_2 = null;
            if (!empty($err_Msg_2)) {
                $error_CRUD = $err_Msg_2 . ', Proses Approve Kabid Keberatan gagal....!!!';
                $res_msg    = $error_CRUD;
                $res_code   = 500;
            } else {
                $res_msg    = 'Data berhasil diapprove kabid keberatan';
                $res_code   = 200;
            }
    ...
    function btn_file($field, $id_ppo) { 
        $url = active_module_url() . 'bid_keberatan_pembetulan_kabid/openblob_permo/' . $field . '/' . $id_ppo;

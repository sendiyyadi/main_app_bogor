            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-0">MONITORING PERMOHONAN PELAYANAN ONLINE</h4>
                        <div class="page-title-right" id="test">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item">
                                    <a href="javascript: void(0);">Tool PBB</a>
                                </li>
                                <li class="breadcrumb-item active">Monitoring Permohonan Pelayanan Online</li>
                            </ol>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="d-flex align-items-center gap-2 mb-2">
                                ...
                                <div class="input-group w-auto">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text rounded-end-0">Sub Jns Pelayanan</span>
                                    </div>
                                    <div class="controls">
                            ...
                            <div class="d-flex align-items-center gap-2 mb-2">
                                ...
                                <button type="button" id="btnshow_rpt" data-tipelaporan="export" class="btn btn-success waves-effect waves-light">Cetak XLS <i class="uil uil-print ms-2"></i></button>
                            </div>
...
<script>
    ...
    function reload_grid() {
        ...
        var sub_jns_ply = $('#sub_jns_ply').val();
        var thn_ply = $('#thn_ply').val();
        var bundel_ply = $('#bundel_ply').val();
        var urut_ply = $('#urut_ply').val();
        var nop = $('#nop').val();
        var sts_kd = $('#status_kd').val();
        var kec_id = $('#kd_kecamatan').val();
        var kel_id = $('#kd_kelurahan').val();

        var params = {
            tgl_fr : tgl_fr,
            tgl_to : tgl_to,
            jns_ply : jns_ply,
            sub_jns_ply : sub_jns_ply,
            thn_ply : thn_ply,
        ...
        oTable.fnReloadAjax("<?php echo active_module_url();?>monitoring_permohonan_online_upt/grid/?"+data_params);
    }

    function f_dtl(id) {
        window.location = '<?php echo active_module_url("monitoring_permohonan_online_upt/detail"); ?>'+id;
    }

    function search_sub_jns(jns_id) {
 
        $.ajax({
            url: "<?php echo active_module_url()?>monitoring_permohonan_online_upt/search_sub_jns/"+jns_id,
            success: function (j) {
                var data = $.parseJSON(j);
                var select = $('#sub_jns_ply');

                select.html("");
                $.each(data, function(i, val){
                    select.append($('<option />', { value: val['ID'], text: val['NM_SUB_JENIS_PELAYANAN'] }));
                });
            },
    ...
    function search_kelurahan(kec_id) {
 
        $.ajax({
            url: "<?php echo active_module_url()?>monitoring_permohonan_online_upt/search_kelurahan/"+kec_id,
            success: function (j) {
                var data = $.parseJSON(j);
    ...
    function show_rpt(rpt, tipelaporan) {
        ...
        if (tipelaporan != 'export') {
            ...
            var url = '<?= active_module_url($this->router->fetch_class()); ?>cetak/xls/export/?' + data;
            window.open(url);
        }
    ...
    $(document).ready(function() {
        ...
        oTable = $('#table1').dataTable({
            ...
            "aoColumnDefs": [
                ...
                { "aTargets": [9], "bSearchable": false,  "bVisible": true,  "sWidth": "30px", "sClass": "", "bSortable": false,
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo active_module_url();?>monitoring_permohonan_online_upt/grid"
        });
        ...
        $("[id=btnshow_rpt]").click(function() {
            ...
            show_rpt(rpt, $(this).data('tipelaporan'));
        });

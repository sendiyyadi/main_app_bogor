                            ...
                            <div class="d-flex align-items-center gap-2 mb-2">
                                <div class="input-group w-auto">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text rounded-end-0">Tgl Apr PKP / Loket</span>
                                    </div>
                                    <div class="controls">
...
<script>
    ...
    function reload_grid() {
        ...
        oTable.fnReloadAjax("<?php echo active_module_url();?>pnt_pembetulan_verif/grid/?"+data_params);
    }

    function f_dtl(id) {
        window.location = '<?php echo active_module_url("pnt_pembetulan_verif/detail"); ?>'+id;
    }

    function f_edit(id, sts) {
        if(sts == 'H') {
            window.location = '<?php echo active_module_url("pnt_pembetulan_verif/edit"); ?>'+id;
        } else {
            alert('Tidak bisa edit data. Data telah di Approve'); return false;
        }
    }
    ...
    function f_ctk(id, sts) {
        ...
        var url = '<?php echo active_module_url(); ?>pnt_pembetulan_verif/cetak_sk/' + id;
    ...
    function f_ctk_sk(id, sts) {
        var url = '<?php echo active_module_url(); ?>pnt_pembetulan_verif/cetak_draft_sk/' + id;
        var winparams = 'width=' + screen.width + ',height=' + screen.height + ',directories=0,titlebar=0,toolbar=0,location=0,status=0,menubar=0,scrollbars=no,resizable=no';
        window.open(url, 'SK Pembetulan', winparams);
    }
    ...
    $(document).ready(function() {
        ...
        oTable = $('#table1').dataTable({
            ...
            "aoColumnDefs": [
                ...
                { "aTargets": [2], "bSearchable": true, "bVisible": true, "sWidth": "65px", "sClass": "" },
                ...
                { "aTargets": [10], "bSearchable": false,  "bVisible": true,  "sWidth": "115px", "sClass": "",
                    "mRender": function( data, type, full) {
                        var edt = '<button class="btn btn-danger rounded-circle me-1" onclick="f_edit(\''+full[0]+'\', \''+full[11]+'\')" type="button" '+
                        ...
                        var ctk = '<button class="btn btn-info rounded-circle me-1" onclick="f_ctk(\''+full[0]+'\', \''+full[11]+'\')" type="button" '+
                                  'data-toggle="tooltip" data-placement="top" title="Cetak SK">'+
                                  '<i class="fas fa-print"></i></button>';
                        var ctk_draft = '<button class="btn btn-info rounded-circle me-1" onclick="f_ctk_sk(\''+full[0]+'\', \''+full[11]+'\')" '+
                                        'type="button" data-toggle="tooltip" data-placement="top" title="Cetak Draft SK">'+
                                        '<i class="fas fa-print"></i></button>';
                        return edt + dtl + ctk_draft ; //+ ctk;
                    }
                },
            ...
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": "<?php echo active_module_url();?>pnt_pembetulan_verif/grid"
        });

            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-0"><?php if ($this->uri->segment(3) == 'detail') echo 'DETAIL '; ?>VERIFIKASI PENETAPAN MUTASI HABIS</h4>
                        <div class="page-title-right" id="test">
                            <ol class="breadcrumb m-0">
                                <li class="breadcrumb-item">
                                    <a href="javascript: void(0);">Tool PBB</a>
                                </li>
                                <li class="breadcrumb-item active">Verif Penetapan Mutasi Habis</li>
                            </ol>
                        </div>
                    </div>
                </div>
...
function f_hapus_detail(id_dob) {
    ...
    if (hapus == true) {
        $.ajax({
          url: '<?php echo active_module_url('pnt_mutasi_habis_verif'); ?>delete_dtl_bng/' + id_dob,
          async: false,
          success: function (j) {
            // var data = $.parseJSON(j);
            alert(j);
...
function f_hapus_detail_fas(id_fas) {
    ...
    if (hapus == true) {
        $.ajax({
          url: '<?php echo active_module_url('pnt_mutasi_habis_verif'); ?>delete_dtl_fas_bng/' + id_fas,
          async: false,
          success: function (j) {
            // var data = $.parseJSON(j);
            alert(j);
...
$(document).ready(function() {
  ...
  $('#myform').on('submit', function(e) {
      ...
      $.ajax({
          url: "<?php echo active_module_url() ?>pnt_mutasi_habis_verif/approve_permohonan/",
          type: 'POST',
          data: data,
          processData: false,
          contentType: false,
          ...
          success: function(res) {
              ...
              if (res.result == 400) {
                  ...
                  Swal.fire({
                      ...
                      window.location = '<?php echo active_module_url("pnt_mutasi_habis_verif"); ?>';
                  });
              }
          },
          complete: function () {
  ...
  $('#btn_save_dsp').click(function() {
      ...
      $.ajax({
          url: '<?php echo active_module_url(); ?>/pnt_mutasi_habis_verif/save_data_subjek_pajak/',
          type: "POST",    
          data: {
            id_ppo: id_ppo,
            ktp_wp: ktp_wp,
  ...
  $('#btn_save_dop').click(function() {
      ...
      $.ajax({
          url: '<?php echo active_module_url(); ?>/pnt_mutasi_habis_verif/save_data_objek_pajak/',
          type: "POST",    
          data: {
            id_ppo: id_ppo,
            ktp_wp: ktp_wp,
  ...
  $('#btn_hitung_njop_bng').click(function(e) {
    ...
    $.ajax({
        url: '<?php echo active_module_url(); ?>/pnt_mutasi_habis_verif/hitung_njop_bng/',
        type: "POST",    
        data: {
          id_ppo: id_ppo,
          paramm: paramm,
  ...
  $('#btn_hitung_njop_bumi').click(function(e) {
      ...
      $.ajax({
          url: '<?php echo active_module_url(); ?>/pnt_mutasi_habis_verif/hitung_njop_bumi/',
          type: "POST",    
          data: {
            id_ppo: id_ppo,
            luas_bumi: luas_bumi,
  ...
  $('#btn_tolak_ply').on('click', function () {
      ...
      Swal.fire({
          ...
          preConfirm: (ket_tolak_ply) => {
              ...
              return $.ajax({
                  url: '<?php echo active_module_url() ?>pnt_mutasi_habis_verif/tolak_ke_pelayanan/',
                  type: 'POST',
                  dataType: 'text',
                  data: {
                      id_ppo: id_ppo,

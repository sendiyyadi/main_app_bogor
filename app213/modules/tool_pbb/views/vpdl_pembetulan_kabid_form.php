                                    ...
                                    <div class="col-md-12 ">
                                      ...
                                      <div id="row_bawah" class="">
                                        ...
                                        <div class="row" style="margin-top:40px">
                                          <div class="col-md-12">
                                            <?php if ($this->uri->segment(3) != 'detail') { ?>
                                            <button class="btn btn-success" id="btn_batal" type="submit" disabled>APPROVE KABID PENDANIL</button>
...
function f_hapus_detail(id_dob) {
    ...
    if (hapus == true) {
        $.ajax({
          url: '<?php echo active_module_url('pdl_pembetulan_kabid'); ?>delete_dtl_bng/' + id_dob,
          async: false,
          success: function (j) {
            // var data = $.parseJSON(j);
            alert(j);
...
function f_hapus_detail_fas(id_fas) {
    ...
    if (hapus == true) {
        $.ajax({
          url: '<?php echo active_module_url('pdl_pembetulan_kabid'); ?>delete_dtl_fas_bng/' + id_fas,
          async: false,
          success: function (j) {
            // var data = $.parseJSON(j);
            alert(j);
...
$(document).ready(function() {
  ...
  $('#myform').on('submit', function(e) {
      ...
      $.ajax({
          url: "<?php echo active_module_url() ?>pdl_pembetulan_kabid/approve_permohonan/",
          type: 'POST',
          data: data,
          processData: false,
          contentType: false,
          ...
          success: function(res) {
              ...
              if (res.result == 400) {
                  ...
                  Swal.fire({
                      ...
                      window.location = '<?php echo active_module_url("pdl_pembetulan_kabid"); ?>';
                  });
              }
          },
          complete: function () {
  ...
  $('#btn_tolak').on('click', function() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "<?php echo active_module_url() ?>pdl_pembetulan_kabid/tolak_permohonan/",
                type: 'POST',
                data: data,
                processData: false,
                contentType: false,
  ...
  $('#btn_save_dsp').click(function() {
      ...
      $.ajax({
          url: '<?php echo active_module_url(); ?>/pdl_pembetulan_kabid/save_data_subjek_pajak/',
          type: "POST",    
          data: {
            id_ppo: id_ppo,
            ktp_wp: ktp_wp,
  ...
  $('#btn_save_dop').click(function() {
      ...
      $.ajax({
          url: '<?php echo active_module_url(); ?>/pdl_pembetulan_kabid/save_data_objek_pajak/',
          type: "POST",    
          data: {
            id_ppo: id_ppo,
            ktp_wp: ktp_wp,
  ...
  $('#btn_hitung_njop_bng').click(function(e) {
    ...
    $.ajax({
        url: '<?php echo active_module_url(); ?>/pdl_pembetulan_kabid/hitung_njop_bng/',
        type: "POST",    
        data: {
          id_ppo: id_ppo,
          paramm: paramm,
  ...
  $('#btn_hitung_njop_bumi').click(function(e) {
      ...
      $.ajax({
          url: '<?php echo active_module_url(); ?>/pdl_pembetulan_kabid/hitung_njop_bumi/',
          type: "POST",    
          data: {
            id_ppo: id_ppo,
            luas_bumi: luas_bumi,
  ...
  $('#btn_tolak_ply').on('click', function () {
      ...
      Swal.fire({
          ...
          preConfirm: (ket_tolak_ply) => {
              ...
              return $.ajax({
                  url: '<?php echo active_module_url() ?>pdl_pembetulan_kabid/tolak_ke_pelayanan/',
                  type: 'POST',
                  dataType: 'text',
                  data: {
                      id_ppo: id_ppo,
